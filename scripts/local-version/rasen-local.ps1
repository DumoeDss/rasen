...
try {
  ...
  Write-Error "Rasen local runtime returned invalid JSON: $($_.Exception.Message)"
  if ($MyInvocation.InvocationName -eq '.') { return }
  exit 1
}

if (-not (Test-Path -LiteralPath $runtime.rasenExecutable -PathType Leaf)) {
  Write-Error "Prepared Rasen executable does not exist: $($runtime.rasenExecutable)"
  if ($MyInvocation.InvocationName -eq '.') { return }
  exit 1
}
...
try {
  ...
  & $runtime.rasenExecutable @RasenArgs
