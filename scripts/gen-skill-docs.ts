...
const SKILLS_DIR = path.join(ROOT, 'skills', 'gstack');
...
function generatePreambleBash(_ctx: TemplateContext): string {
...
// generateUpgradeCheck — REMOVED (OpenSpec uses openspec init --force for updates)
// generateLakeIntro — REMOVED (gstack branding)
...
// generateContributorMode — REMOVED (gstack-specific feedback mechanism)
...
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.openspec/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
function codexSkillName(skillDir: string): string {
  if (skillDir === '.' || skillDir === '') return 'openspec';
  // Don't double-prefix
  if (skillDir.startsWith('openspec-')) return skillDir;
  return `openspec-${skillDir}`;
}
...
function processTemplate(tmplPath: string, host: Host = 'claude'): { outputPath: string; content: string } {
  ...
  // Determine skill directory relative to SKILLS_DIR
  const skillDir = path.relative(SKILLS_DIR, path.dirname(tmplPath));
  ...
  if (host === 'codex') {
    ...
    // Replace remaining hardcoded OpenSpec paths with host-appropriate paths
    content = content.replace(/~\/\.openspec\/bin/g, ctx.paths.binDir);
    content = content.replace(/~\/\.openspec\/skills/g, ctx.paths.skillRoot);
    content = content.replace(/\.openspec\/skills/g, ctx.paths.localSkillRoot);
    content = content.replace(/\.openspec\/review/g, '.agents/skills/openspec/review');
...
function findTemplates(): string[] {
  ...
  // Check for root-level SKILL.md.tmpl in skills/
  const rootTmpl = path.join(SKILLS_DIR, 'SKILL.md.tmpl');
  if (fs.existsSync(rootTmpl)) templates.push(rootTmpl);

  if (!fs.existsSync(SKILLS_DIR)) return templates;
