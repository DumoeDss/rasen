...
| Issue Board / Detail 只读界面（`/s/:storeId/issues[/:issueId]`） | `packages/ui/src/components/IssueBoardPage.tsx`（五固定 phase 泳道 + 不完整性通告条 + MemberChips 纯过滤不持久化）、`IssueCard.tsx`（标题/health 徽章/progress 对/attention 首条）、`IssueDetailPage.tsx`（§9.2 七段）、`issue-vocabulary.ts`（**闭词汇→i18n 键的字面查表，UI 侧唯一映射层：不派生 phase/health/progress/determination**）；路由在 `app.tsx`（store 空间专属，无 `/p/` 对，不进 `SWITCHABLE_SECTIONS`），导航项在 `Layout.tsx`（store 空间才渲染，按路径前缀自标 aria-current）；wire 镜像 + client 三方法在 `packages/ui/src/api/{types,client}.ts` |
