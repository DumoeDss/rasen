...
| Issue 系统分配身份 / selector 解析 / V1 投影 / 不确定发布恢复 | `src/core/store/issues/identity.ts`（三入口：`allocateIssueIdentity` 分配 UUIDv4 + 派生 `ISS-…` key，`projectStoredIssueIdentity` 以 Store UID 将 V1 legacy id 投影为 UUIDv5，`resolveIssueSelector` 对 UID/key/slug/alias fail-closed）；`src/core/store/issues/module.ts`（record commit point + `issue_publication_indeterminate` UID/key/`retrySafe:false`）；`src/core/management-api/stores.ts`（公共 diagnostic/warning 投影，storageKey/路径/raw cause 不出 HTTP/UI/CLI JSON） |
| Issue V1/V2 record 与 owned-resource codec | `src/core/store/issues/records.ts`（V1 字节兼容 + strict V2 `identity`）、`plans.ts`/`acceptance.ts`（V1 digest 保持 + 新 V2 `issueUid` owner）；`module.ts` 在 allocation/selector lock 内 resolve，再按 UID 加锁并按实际 storage locator 读写，`query/issues-read.ts` 投影后按 UID 分组 |
...
| Issue 投影 HTTP 端点（三条 GET，flat Store-aggregate 族） | `src/core/management-api/stores.ts`（`handleStoreIssueProjections`/`handleStoreIssueProjection`/`handleStoreIssueAttention` = compose 的透传，零缓存零派生；单 Issue projection 先确认 selector 命中，真正缺失返回 `404 issue_not_found`；`mapThrown` 的 identity 状态表统一 400/404/409，另将 `issue_attention_unknown_issue` 映到 404）+ `router.ts`（`/api/v1/stores/issue-projections|issue-projection|issue-attention`，`space=store:<uid>` 必填、每请求重解 run-state）+ `wire-types.ts`（三 alias，unwrapped passthrough） |
...
| Issue Board / Detail 界面（Store canonical `/s/:storeId/issues[/:issueSelector]`） | wire mirror 在 `packages/ui/src/api/types.ts`（structured identity + V1/V2 resource unions），client 在 `api/client.ts`（兼容 query key `issueId` 只承载 selector）；`IssueBoardPage.tsx` 创建只发 title、消费服务端 UID/key 并全量重读；`IssueCard.tsx` 显示 key+title 且以 UID 作 key/link；`IssueDetailPage.tsx` 接受 UID/key/slug/legacy selector，成功后 replace 为 UID route；`LinkChangeDialog.tsx` create→plan/partial recovery 使用创建响应 UID/key，publication-indeterminate 则锁死当前 dialog 的 fresh create 并仅按 recovery UID 检查/关联；`issue-provenance.ts` 展示 identity 四事实且绝不暴露 storage key；闭词汇仍只由 `issue-vocabulary.ts` 映射。路由在 `app.tsx`（Store 专属，无 `/p/` 对，不进 `SWITCHABLE_SECTIONS`） |
