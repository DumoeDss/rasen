...
- **Store aggregate 两族**：维护中的是 **flat 族**（`/api/v1/stores/{projects,target-lines,changes,issues,issue,issue-state,issue-references,execution-plan}` + Phase 7 的 `issue-projections`/`issue-projection`/`issue-attention`/`change-issue-links`），`space=store:<uid>` 寻址、`stores.ts` 的 `run()`/`statusForIssueCode` 统一错误映射、wire 类型 = core 类型 alias 的 unwrapped passthrough。公共 Issue projector 同时剥离 unreadable/storage-mismatch fallback 中的 locator、绝对路径与 raw diagnostic；Execution Plan projector 必须同时接收 public Issue summary，以 verified UID（或 unavailable marker）决定顶层 `issueId` 与 problem `itemId`，绝不白名单 core plan 自带的 storage fallback；创建的 `issue_publication_indeterminate` 以 UID/key + `retrySafe:false` recovery 穿 flat HTTP/CLI JSON。`stores-routes.ts` 的 0.1.7-port 参数化族已弃用，只保留兼容修复：option-shaped legacy alias 必须置于 argv `--` 之后，且同样透传上述结构化 recovery。
- **Issue 投影三条读（g-001）**：`handleStoreIssueProjections`/`handleStoreIssueProjection`/`handleStoreIssueAttention` 是 `src/core/issue-read/` 三个 compose 函数的透传——服务端零派生、零翻译层、零缓存，每请求重解 run-state（所以两次相同 GET 之间的 Store 变更不需要任何失效步骤就被第二次读到）。review **不设第四端点**：它是单 Issue 读载荷的 `review` 键（同一 status 同一派生）。单 Issue projection 在 compose 前确认 selector 命中，真正缺失返回 `404 issue_not_found`；`mapThrown` 的 identity 状态表统一 400/404/409，并把不属于 `StoreIssueErrorCode` 闭 union 的 `issue_attention_unknown_issue` 映到 404，避免客户端错误落成 500 `store_query_failed`。
