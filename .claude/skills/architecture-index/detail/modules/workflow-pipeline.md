...
- **Store aggregate 两族**：维护中的是 **flat 族**（`/api/v1/stores/{projects,target-lines,changes,issues,issue,issue-state,issue-references,execution-plan}` + Phase 7 g-001 的三条投影读 `issue-projections`/`issue-projection`/`issue-attention`），`space=store:<uid>` 寻址、`stores.ts` 的 `run()`/`statusForIssueCode` 统一错误映射、wire 类型 = core 类型 alias 的 unwrapped passthrough。`stores-routes.ts` 的 0.1.7-port 参数化族（手抄 `Wire*` + 粗错误映射）**弃用，不扩展**。
