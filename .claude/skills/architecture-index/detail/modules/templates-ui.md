...
- **App 路由**：Board / Config / Pipelines / Archive / Audit / Spaces / Workflows / Profiles / TaskDetail / IssueBoard / IssueDetail / Operations / UnlinkedChanges（后四者是 store-only `/s/:storeId/issues[/:issueId]|operations|unlinked-changes`；无 `/p/` 对且不进 `SWITCHABLE_SECTIONS`）+ 懒加载 PipelineCanvas。Issue Board/Detail 是只读投影呈现，闭词汇→label 查表在 `components/issue-vocabulary.ts`，不派生任何轴。
