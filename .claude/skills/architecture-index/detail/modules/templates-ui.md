...
- **App 路由 / canonical home**：`/p/:projectId/board` + `/p/:projectId/task/:change` 是 project-only Change/Task 面；Store canonical home 是 `/s/:storeId/issues[/:issueSelector]`，generated link 必用 Issue UID，key/slug/legacy deep link 只作兼容 selector，成功 resolve 后 replace 为 UID route。另有 Store-only `/operations|unlinked-changes`，均无 `/p/` 对且不进 `SWITCHABLE_SECTIONS`。`/s/:storeId/board` replace-redirect 到 Issues，Store Task URL replace-redirect 到 Operations。类型感知 home/switch 矩阵集中在 `store/use-space.ts` 的 `spaceHomeHref`/`spaceSwitchHref`，由 bootstrap/switcher/space rows/create success 共用。
- **Issue Board / provenance**：`IssueBoardPage.tsx` 的创建表单只有 title，`createStoreIssue` 返回的 UID/key 才能进入成功导航/展示；成功后全量重读 projections/attention/projects，绝不从 title 造 identity 或乐观伪造卡片。它与 `IssueDetailPage.tsx` 用 Store selector + Issue selector keyed state child 隔离同组件路由切换；Detail resolve 后以 UID canonicalize。`IssueCard.tsx` 显示 key+title，以 UID 作为 component identity、Detail link 与 evidence link 基址；`issue-provenance.ts` 在原六个展示族上呈现 UID/key/slug/aliases，只复制 `git|runtime` locator，不暴露 storage key、不派生状态。闭词汇→label 查表仍在 `components/issue-vocabulary.ts`。
- **Cutover disposition**：`BoardPage.tsx` 只服务 project，不含 Store roster/member branch；`StoreIssuesView`、`StoreAggregateBoard`、`RunningSessionsMenu` 及专属 tests/styles/locale 已删除。Store execution 由 Operations 拥有，project live indicator 与 Run control 由 project Board/Task Detail 拥有。
- **Store Operations**：`components/OperationsPage.tsx` 从 Store roster 扇出 Store/member Sessions 与 rooted-member Runs，保留每个 Run 的精确 selector；`operations-model.ts` 纯分类 active/abnormal/settled 并只用 frozen execution + D1 link payload 做 Change/Issue attribution，cwd 仅作 locator；`OperationsSection.tsx` 暴露 Task Detail 与 Store 页共用的 server-projected Run panel（resume/retry/confirm-first stop、Record version、committed-view replacement）。member failure 独立、cursor opaque、只在 live work 存在时 3 秒 polling。
- **Unlinked Changes**：`components/UnlinkedChangesPage.tsx` 只把 D1 的 `unlinked/attachable` occurrence 作为可写行，linked 与 unknown 分区显示；`LinkChangeDialog.tsx` create preview 只要求 title，随后消费创建响应 UID 发布首个 V2 plan，attach 选项也以 UID 为值；复制所有 plan-node 字段并发送 `expectedRevisionId`。stale conflict 刷新 committed truth；create 成功而首 plan 失败时以返回 key 命名、返回 UID 恢复，保留 Issue 且不做补偿删除；publication-indeterminate 保留 recovery UID/key、锁死当前 dialog 的二次 fresh create，并只提供按 canonical UID 的检查/关联动作。
