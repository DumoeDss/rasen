...
- **Issue Board / provenance**：`IssueBoardPage.tsx` 用既有 `createStoreIssue` 写缝提供 Store Issue 新建入口，成功后全量重读 projections/attention/projects，绝不乐观伪造卡片；它与 `IssueDetailPage.tsx` 用 selector（Detail 再加 Issue id）keyed state child 隔离同组件路由切换。`IssueCard.tsx` 的轴/attention evidence link 进入 Detail 稳定 fragment；`issue-provenance.ts` 把既有 payload 分成 record、plan/projection、acceptance/review、runtime、delivery、attention 六个展示族，只复制 `git|runtime` locator，不派生状态。闭词汇→label 查表仍在 `components/issue-vocabulary.ts`。
...
- **Unlinked Changes**：`components/UnlinkedChangesPage.tsx` 只把 D1 的 `unlinked/attachable` occurrence 作为可写行，linked 与 unknown 分区显示；`LinkChangeDialog.tsx` preview-confirm 组合既有 Issue create/complete-plan publish，复制所有 plan-node 字段并发送 `expectedRevisionId`。stale conflict 刷新 committed truth；create 成功而首 plan 失败时保留 Issue 并暴露 attach recovery，不做补偿删除。
