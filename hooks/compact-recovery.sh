...
1. Run: rasen pipeline resume <change> --json  (add --store <id> for store-scoped changes)
   It reports the sessionHandoff pointer and each stage's latest handoff document.
2. Read the handoff documents (rasen/changes/<change>/handoff/) FIRST — they are curated
