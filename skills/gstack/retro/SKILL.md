...
cat ~/.openspec/greptile-history.md 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `~/.openspec/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Eureka Moments (if logged):** Read `~/.openspec/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
**Note:** Only include the `greptile` field if `~/.openspec/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
> **Note:** Global discovery requires a discovery tool. This feature is pending OpenSpec integration.

Tell the user: "Global retro discovery is not yet available in OpenSpec. Use change-scoped or general retro instead." and stop.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not separate commits for discovery and template — one cohesive commit per feature.
...
```bash
ls -t ~/.openspec/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p ~/.openspec/retros
```

Determine the next sequence number for today:
```bash
today=$(date +%Y-%m-%d)
existing=$(ls ~/.openspec/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.openspec/retros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.openspec/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
