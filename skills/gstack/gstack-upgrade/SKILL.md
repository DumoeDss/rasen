...
[ -z "$_AUTO" ] && _AUTO=$(~/.openspec/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.openspec/bin/gstack-config set auto_upgrade true
```
Tell user: "Auto-upgrade enabled. Future updates will install automatically." Then proceed to Step 2.

**If "Not now":** Write snooze state with escalating backoff (first snooze = 24h, second = 48h, third+ = 1 week), then continue with the current skill. Do not mention the upgrade again.
```bash
_SNOOZE_FILE=~/.openspec/update-snoozed
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `auto_upgrade: true` in `~/.openspec/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
~/.openspec/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.openspec/bin/gstack-config set update_check true` to re-enable."
...
elif [ -d "$HOME/.openspec/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.openspec/repos/gstack"
...
```bash
mkdir -p ~/.gstack
echo "$OLD_VERSION" > ~/.openspec/just-upgraded-from
rm -f ~/.openspec/last-update-check
rm -f ~/.openspec/update-snoozed
```
...
```bash
~/.openspec/bin/gstack-update-check --force 2>/dev/null || \
