...
version: 1.0.0
description: |
  Build a throwaway prototype that answers one design question. Two branches:
  LOGIC (a tiny interactive terminal probe for a state machine / model) and UI
  (several radically different variations on one route, switchable via a URL
  param). Six shared rules, and capture the answer when done.
  Use when the user wants to sanity-check whether a state model or logic feels
  right, or explore what a UI should look like.
  Proactively suggest when a design decision is stuck on paper and a small
  runnable experiment would settle it faster than more discussion.
allowed-tools:
  - Bash
  - Read
  - Write
