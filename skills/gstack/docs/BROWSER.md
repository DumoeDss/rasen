...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.openspec/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.openspec/browse-console.log`
- Network: `.openspec/browse-network.log`
- Dialog: `.openspec/browse-dialog.log`
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.openspec/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.openspec/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.openspec/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.openspec/browse.json` | Path to state file (CLI passes to server) |
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.openspec/browse.json`, sends HTTP to the server, prints response. |
