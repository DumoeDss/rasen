...
**Design doc check:**
```bash
SLUG=$(~/.openspec/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.openspec/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.openspec/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
HANDOFF=$(ls -t ~/.openspec/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```
Write to `~/.openspec/projects/$SLUG/$USER-$BRANCH-ceo-handoff-$DATETIME.md`:
```markdown
...
```bash
SLUG=$(basename "$(git remote get-url origin 2>/dev/null)" .git 2>/dev/null || basename "$(pwd)") && mkdir -p ~/.openspec/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.openspec/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.openspec/projects/$SLUG/ceo-plans/{old-plan}.md ~/.openspec/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.openspec/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.openspec/analytics/spec-review.jsonl 2>/dev/null || true
```
...
rm -f ~/.openspec/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
After displaying the Review Readiness Dashboard, recommend the next review(s) based on what this CEO review discovered. Use the dashboard to see which reviews have already been run this session.

**Recommend /plan-eng-review unless the user opted out of eng review** — if the user opted out of eng review this session, do not recommend it. Otherwise, eng review is the required shipping gate. If this CEO review expanded scope, changed architectural direction, or accepted scope expansions, emphasize that a fresh eng review is needed. If an eng review was run earlier but the plan has changed substantially since this CEO review, note that it may be stale and should be re-run.

**Recommend /plan-design-review if UI scope was detected** — specifically if Section 11 (Design & UX Review) was NOT skipped, or if accepted scope expansions included UI-facing features. If a design review was run earlier but the plan has changed substantially since, note that it may be stale. In SCOPE REDUCTION mode, skip this recommendation — design review is unlikely relevant for scope cuts.
...
- **B)** Keep in `~/.openspec/projects/` only (local, personal reference)
- **C)** Skip
