...
version: 1.0.0
description: |
  Test-driven development done so the tests are worth keeping: the red → green
  loop plus what a good test is (behaviour through public interfaces), where
  tests go (seams), the three anti-patterns (implementation-coupled /
  tautological / horizontal-slicing), and the rules of the loop.
  Use when the user wants to build features or fix bugs test-first, mentions
  "red-green-refactor", or wants integration tests.
  Proactively suggest when new behaviour is about to be written without a
  failing test, or when existing tests keep breaking on refactors.
allowed-tools:
  - Read
  - Write
  - Edit
