...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to Rasen'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • rasen-* skills, invoked by their canonical name'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('rasen-new-change')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('rasen-continue-change')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('rasen-apply-change')}    ${chalk.dim('Implement tasks')}`,
    '',
