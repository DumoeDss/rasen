...
/**
 * Generates Zsh completion scripts for the Rasen CLI.
 * Follows Zsh completion system conventions using the _rasen function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _rasen_${this.sanitizeFunctionName(cmd.name)}`);
    ...
    return `#compdef rasen

# Zsh completion script for Rasen CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "rasen command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
`;
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _rasen_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_rasen_complete_changes'";
      case 'spec-id':
        return "'*: :_rasen_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_rasen_complete_items'";
      case 'schema-name':
        return "'*: :_rasen_complete_schemas'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
  ...
  private generateIndexedPositionalSpec(
    ...
    switch (positional.type) {
      case 'change-id':
        return `'${index}${separator}${name}:_rasen_complete_changes'`;
      case 'spec-id':
        return `'${index}${separator}${name}:_rasen_complete_specs'`;
      case 'change-or-spec-id':
        return `'${index}${separator}${name}:_rasen_complete_items'`;
      case 'schema-name':
        return `'${index}${separator}${name}:_rasen_complete_schemas'`;
      case 'path':
