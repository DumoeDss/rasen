...
/**
 * Generates Bash completion scripts for the Rasen CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for Rasen CLI
...
${helpers}
complete -F _rasen_completion rasen
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_rasen_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_rasen_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_rasen_complete_items`);
        break;
      case 'schema-name':
