...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_rasen'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_rasen'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  /**
   * Check if .zshrc has Rasen configuration markers
   *
  ...
  private async hasZshrcConfig(): Promise<boolean> {
    try {
      ...
      return this.resolveAllPresentMarkers(content).length > 0;
    } catch {
      return false;
    }
  }
   ...
   * present in the given .zshrc content.
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed Rasen configuration from ~/.zshrc');
      }
