...
export function resolveDispatchRoute(
  ...
  if (options.externalInference) {
    ...
    // dispatch adapter cannot serve one. Guarding on 'canDispatch' rather
    // than on host === 'unknown' is load-bearing: a recognized-but-non-
    // dispatching host would otherwise reach a print bridge here.
