...
// Accounting the archive engine writes directly onto `.openspec.yaml` via raw
// YAML (see `captureArchiveQuality` in `archive-engine.ts`), bypassing this
// schema entirely. `ChangeMetadataSchema` is strict so a misspelled or
// unexpected field is still reported, but this engine-owned block must stay
// readable — including historical records that also carry `rulesExtracted`
// (see e.g. `rasen/changes/archive/2026-07-06-add-context-handoff/.openspec.yaml`).
// `.passthrough()` keeps it tolerant of any other field the engine adds later.
const ArchiveQualitySummarySchema = z
  .object({
...
export const ChangeMetadataSchema = z
  .object({
    ...
    quality: ArchiveQualitySummarySchema.optional(),
  })
  .strict();
