...
- \`goal-run.json\` (authoritative) — the per-round records: \`{round, score?, measurePassed?, evaluateSatisfied?, detail?, gaps?, error?, gitTreeFingerprint}\`. This is ephemera (\`file-placement\` capability): find it in the execution root's ephemera directory (\`ephemeraDir\` from \`rasen status --change <n> --json\`, or the resolved location the LEAD's dispatch prompt names); sticky-legacy: a run artifact that already lives in the legacy \`workDir\` or the change directory is read there instead.
...
- Read \`goal-run.json\` (from the resolved ephemera location per the Input section above) as the source of truth; do not infer outcomes from the work product alone.
