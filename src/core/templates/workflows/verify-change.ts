...
9. **Save Report**
   ...
   Write the full verification result to \`verification-report.md\` in the change's evidence directory (\`evidenceDir\` from \`rasen status --change <name> --json\`; sticky-legacy: a file that already lives in the legacy \`workDir\` or the change directory is used in place) — the summary scorecard, the \`VERIFY VERDICT:\` status line, and the grouped findings (Blocker/Major/Minor/Trivial, each with its recommendation). This is verify-change's canonical evidence artifact that \`rasen-ship\`'s pre-flight looks for. Emitting the result only to the conversation is NOT sufficient.
   ...
   **Test-evidence block (only when you ran tests/gates)**: if this verification executed the project's test or gate command(s), append a fingerprinted test-evidence block so \`rasen-ship\`'s evidence-based test-skip gate can honor it — the same schema \`review-cycle-report.md\` records:
   ...
   If this verification did NOT run tests, write no test-evidence block — ship
   then selects and runs only the uncovered checks in its required verification
   scope.
