...
**Work across repositories.** Register other Rasen repos as stores (\`rasen store register <path>\`) or projects (\`rasen store add-project\`), then target them with \`--store <id>\` / \`--project <id>\` on workspace commands. \`rasen store list --json\` shows what's registered. To MOVE a project's planning into a store, \`rasen store adopt [path] --to <store-id>\` migrates its specs/changes and converts the repo to a pointer; \`rasen store eject <project-id> --from <store-id>\` reverses it. \`rasen archive relocate --to <in-repo|store>\` consolidates existing archives — including legacy machine-home ones — into the planning root or the store, and writes no configuration (archives always land in the planning root; there is no destination setting). \`rasen home prune\` cleans orphaned machine-home state (report-only until \`--apply\`). All support \`--dry-run\` and \`--json\` and never touch git.
