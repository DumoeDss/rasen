...
2. **Select the change.** Use the active change being driven (infer from conversation / \`rasen list --json\`; prompt only if genuinely ambiguous). If no change is active, write the document to \`rasen/handoff/<topic-slug>.md\` instead and skip the run-state update.
3. **Write the document** to \`rasen/changes/<name>/handoff/lead-<n>.md\` where \`<n>\` is 1 + the highest existing lead-* number (never overwrite a predecessor). Use the template below.
4. **Update run-state** (\`rasen/changes/<name>/auto-run.json\`): set top-level \`sessionHandoff\` to \`{ "path": "handoff/lead-<n>.md", "n": <n>, "pct": <probe pct>, "afterStage": "<last completed stage>", "at": "<ISO timestamp>" }\` — \`n\` is the relay generation and matches the document number (a record without \`n\` reads as generation 1). Create the file with just that field if no run-state exists yet.
...
**Bootstrap prompt — file indirection, never bare quoting.** Write the successor's first instruction to \`rasen/changes/<name>/handoff/relay-prompt.txt\`:

\`\`\`
You are the successor session (generation <n+1>) for change <name>.
1. Read rasen/changes/<name>/handoff/lead-<n>.md — it is your predecessor's distillate; do not re-litigate its decisions.
...
A worker told to hand off mid-stage writes \`rasen/changes/<name>/handoff/<role>-<n>.md\` with the same template, then returns the structured \`HANDOFF\` result to the LEAD. Workers NEVER update run-state — the LEAD does that accounting (single-writer invariant).
