...
1. **Probe first.** Run \`rasen agent context --latest --json\` and report \`{ contextTokens, limit, pct, remainingTokens }\` to the user (\`remainingTokens\` is what an absolute \`{ remainingTokens: N }\` threshold compares against directly). This is informational — the user decides; do not refuse to hand off below any threshold. The probe may instead answer \`{"available": false, "reason": "no-transcript"|"unsupported-host", "detail": "..."}\` at exit \`0\` with none of those fields — \`unsupported-host\` means this session's harness has no context-probe adapter, so no reading exists to take. That NEVER blocks a handoff: report the \`detail\` verbatim as "occupancy unmeasurable on this host", omit \`pct\` from step 4's record (the field is optional), and continue with the rest of this flow unchanged. Do not force \`RASEN_AGENT_RUNTIME\` to make it answer — on a harness with no probe adapter that yields a DIFFERENT session's occupancy, not yours. A THIRD outcome is \`available: true\` carrying \`"window": "unknown"\` and NO \`shouldHandoff\` field: the occupancy is REAL but the model's context window is unknown, so \`limit\`, \`pct\` and \`remainingTokens\` are placeholders rather than measurements. Branch on the PRESENCE of \`shouldHandoff\`, never on its falsiness. Report occupancy as \`<contextTokens>/unknown\` — never a \`0.0%\` fraction and never \`remaining=0\`, both of which read as "nearly empty" for a session that may be nearly full — and omit \`pct\` from step 4's record exactly as the \`available: false\` arm does.
2. **Select the change.** Use the active change being driven (infer from conversation / \`rasen list --json\`; prompt only if genuinely ambiguous). If no change is active, write the document to \`rasen/handoff/<topic-slug>.md\` instead and skip the run-state update (this repo-level fallback has no change scope and is unaffected by the rest of this section). Otherwise resolve \`handoffDir\` and \`ephemeraDir\` from \`rasen status --change <name> --json\` — both are always present. Handoff documents and relay prompts land in \`handoffDir\` (\`<changeRoot>/handoff/\`, so they travel with the change); run-state lands in \`ephemeraDir\`. **Sticky-legacy series rule:** when this change's handoff documents already live in the legacy machine-home work directory's \`handoff/\` or in the change directory's \`handoff/\`, keep writing new documents there — one change's handoff series stays in ONE place and is never split across locations. The run-state file follows the same rule independently.
3. **Write the document** to \`<handoffDir>/lead-<n>.md\` (or the legacy location this change's series already uses) where \`<n>\` is 1 + the highest existing \`lead-*\` number — scan the SAME directory you are about to write to, never a different one, or numbering restarts and overwrites a predecessor. Use the template below.
4. **Update run-state** (\`<ephemeraDir>/auto-run.json\`; sticky-legacy: an \`auto-run.json\` that already lives in the legacy work directory or the change directory is updated in place): set top-level \`sessionHandoff\` to \`{ "path": "<absolute path to the document you just wrote>", "n": <n>, "pct": <probe pct>, "afterStage": "<last completed stage>", "at": "<ISO timestamp>" }\` — \`n\` is the relay generation and matches the document number (a record without \`n\` reads as generation 1). OMIT \`pct\` entirely when step 1 reported no measurable fraction (either \`available: false\` or \`"window": "unknown"\`) — the field is optional, and recording the placeholder \`0\` would durably assert that an exhausted session was empty. **Record \`path\` ABSOLUTE.** Run-state and the handoff document now live under different roots (\`ephemeraDir\` vs \`handoffDir\`), so a bare \`handoff/lead-<n>.md\` has no base a reader can trust — \`rasen pipeline resume\` prints this pointer beside \`runStateDir\`, and resolving it there would land in the ephemera directory, where the document does not exist. An older record holding a relative path still resolves against the change directory. Create the file with just that field if no run-state exists yet.
...
**Generation cap.** Before spawning, check the generation: if the new document's \`n\` has reached \`maxRelays\` (the pipeline's resolved handoff config via \`rasen pipeline show <pipeline> --json\`, default 3), do NOT auto-spawn. Present the relay history (the \`lead-*.md\` documents in the resolved handoff directory) and recommend decomposing the change instead — repeated session relays signal work that should be split, not relayed harder.

**Bootstrap prompt — file indirection, never bare quoting.** Write the successor's first instruction to \`<handoffDir>/relay-prompt.txt\` (or the legacy handoff location this change's series already uses — the relay prompt stays with its documents):
...
A worker told to hand off mid-stage writes \`<role>-<n>.md\` into the change's handoff directory — \`handoffDir\` from its dispatch payload, or from \`rasen status --change <name> --json\` — under the same sticky-legacy series rule, with the same template, then returns the structured \`HANDOFF\` result to the LEAD. Workers NEVER update run-state — the LEAD does that accounting (single-writer invariant).
...
export function getHandoffSkillTemplate(): SkillTemplate {
  return {
    name: 'rasen-handoff',
    description:
      'Write a handoff document — probe context usage (rasen agent context), distill decisions / dead ends / eliminated hypotheses / next action to the handoff directory, and record the sessionHandoff pointer so a fresh session or successor worker resumes from the distillate instead of a raw transcript.',
