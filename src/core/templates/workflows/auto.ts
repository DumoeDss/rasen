...
Resolve the effective **gate policy** with precedence **run flag > project config > store config > global config > built-in default**: (1) \`--no-gate\` present on the invocation -> \`off\`, source \`flag\`; else (2) \`autopilot.gates: on|off\` in \`rasen/config.yaml\` (read via the project config the same way other config keys resolve) -> that value, source \`project\`; else (3) \`autopilot.gates: on|off\` in the inherited store's config (when \`rasen/config.yaml\` declares \`store:\` beside local planning) -> that value, source \`store\`; else (4) \`autopilot.gates: on|off\` in the global (machine-wide) config -> that value, source \`global\`; else (5) \`on\`, source \`default\`. Display the resolved policy at run start (e.g. \`Gate policy: off (flag)\`) so it is visible, never silent. Record it ONCE as \`gatePolicy: { effective, source }\` in run-state (Step F) at run start — Step D then reads this recorded value for every gate rather than re-deriving it, and **resume reads it back from run-state so the user does NOT re-pass \`--no-gate\`** on a resumed run. This governs ONLY ordinary gates (\`gate: true\`); a \`gate: 'vet'\` stage ALWAYS pauses regardless of policy — see the guardrail below and the playbook's Step D.
