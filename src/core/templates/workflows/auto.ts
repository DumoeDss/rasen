...
This probe is non-blocking for EVERY host. It answers \`{"available": false, "reason": "no-transcript"|"unsupported-host", "detail": "..."}\` at exit \`0\` — never an error — in two normal cases: a non-Claude LEAD (e.g. a Codex CLI session) with no Claude transcript to read (\`no-transcript\`), and a LEAD whose harness has no context-probe adapter at all (\`unsupported-host\` — there is no reading to take, and reading some other harness's newest transcript would report a DIFFERENT conversation's occupancy as yours). An Oh My Pi LEAD is measurable: it has its own context reader, so an implicit \`--latest\` there reports THAT session's own occupancy. Branch on \`available\`, not on the reason literal and not on the presence of \`pct\`: on \`available: false\`, treat it as "no occupancy signal available", record it if you are tracking your own state (e.g. \`unavailable-<runtime>\`), and proceed exactly as if the threshold had not been reached. Do not treat \`available: false\` as a failure to swallow or retry, do not force \`RASEN_AGENT_RUNTIME\` to make it answer, and do not read a missing \`pct\` as "below threshold".
...
The user may mix runtimes across any host-target pair the shipped adapters support. Routes are derived, not fixed: a host targeting its own runtime is native, and a dispatch-capable host targeting a different dispatch-capable runtime uses the bridge that TARGET declares — today \`codex-exec\` for a Codex worker and \`claude-print\` (through \`rasen agent dispatch\`) for a Claude worker. A host with no dispatch adapter of its own takes the legacy compatibility route instead. Read the resolved route from \`rasen pipeline show <name> --for-execution --json\` rather than assuming a pair. Pipeline stages may also set \`runtime\`, \`sessionReuse\`, \`sandbox\`, \`model\`, and \`effort\`; invocation role flags override those defaults for this run because they were forwarded into the final execution-plan command above. The effective **model** for a stage additionally falls through machine config below the pipeline's own role default: stage \`model\` > pipeline \`agents.<role>.model\` > project config \`models.roles.<role>\` > project config \`models.default\` > global config \`models.roles.<role>\` > global config \`models.default\` > the runtime's own default. \`rasen pipeline show <name> --json\` already resolves this whole chain — read the stage's \`model\`/\`modelSource\` from there rather than re-deriving it.
