...
Resolve the effective **selection policy** with precedence **run flags (\`--auto-compose\` > \`--auto-select\`) > project config > built-in default**: (1) \`--auto-compose\` present on the invocation -> \`compose\`, source \`flag\` (compose is the superset policy — it wins over \`--auto-select\` when both are present, since composing always classifies first); else (2) \`--auto-select\` present -> \`classify\`, source \`flag\`; else (3) \`autopilot.selection: classify|manual|compose\` in \`rasen/config.yaml\` -> that value, source \`config\`; else (4) \`manual\`, source \`default\`. Display the resolved policy at run start alongside the gate policy line (e.g. \`Selection policy: compose (flag)\`) so an opted-in run is never silent about how it will pick a pipeline. This governs ONLY the no-explicit-selector branch of pipeline selection (section 1 below) — an explicit selector always wins regardless of policy, and absent every flag and the config key, selection behavior is exactly 0.1.x (\`manual\`, default \`small-feature\`, no auto-escalation).
...
- The LEAD never executes an unregistered, in-memory DAG: every pipeline it runs — including its own compositions — MUST resolve by name via \`rasen pipeline show <name> --json\` before execution. A composed pipeline is always written to the project pipelines directory and validated (\`rasen validate <name> --type pipeline --json\`) before any stage runs.
