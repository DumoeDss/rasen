...
This probe is non-blocking for EVERY host. It answers \`{"available": false, "reason": "no-transcript"|"unsupported-host", "detail": "..."}\` at exit \`0\` — never an error — in two normal cases: a non-Claude LEAD (e.g. a Codex CLI session) with no Claude transcript to read (\`no-transcript\`), and a LEAD whose harness has no context-probe adapter at all (\`unsupported-host\`, e.g. an Oh My Pi session — there is no reading to take, and reading some other harness's newest transcript would report a DIFFERENT conversation's occupancy as yours). Branch on \`available\`, not on the reason literal and not on the presence of \`pct\`: on \`available: false\`, treat it as "no occupancy signal available", record it if you are tracking your own state (e.g. \`unavailable-<runtime>\`), and proceed exactly as if the threshold had not been reached. Do not treat \`available: false\` as a failure to swallow or retry, do not force \`RASEN_AGENT_RUNTIME\` to make it answer, and do not read a missing \`pct\` as "below threshold".
