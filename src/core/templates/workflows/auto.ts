...
\`autopilot.gates\` is a **mask base**, not the final word on any single gate. Each stage's effective gate resolves as \`pipelines.<name>.gates.<stage>\` (a per-stage config instance, project > store > global) **over** this base (an effective \`off\` suppresses an ordinary gate) **over** the stage definition's own \`gate:\`. Do NOT combine the base with stage definitions yourself: read each stage's already-masked **effective gate** from \`rasen pipeline show <name> --json\` (each stage's \`effectiveGate\`, with the deciding layer in \`gateSource\`) and gate Step D on that resolved value — so a per-stage \`on\` pierces an \`off\` base and a per-stage \`off\` silences one stage under an \`on\` base, exactly as configured. This governs ONLY ordinary gates (\`gate: true\`); a \`gate: 'vet'\` stage ALWAYS pauses regardless of policy — see the guardrail below and the playbook's Step D.
