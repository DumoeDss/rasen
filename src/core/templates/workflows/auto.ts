...
**After the human confirms, continue the dispatch through launch contracts.** When the operator has run \`rasen store issue confirm <issue-id>\` over the reviewed revision (with whatever new revisions their review authored), the LEAD continues as their agent: drive each launchable node the confirm step reported through its \`rasen store issue start --node <id>\` launch contract, honoring the dependency frontier so a downstream node waits for completed work. The confirm is the green light and the revision is the scope — no node outside the confirmed revision's wanted work is started, and a revision published after confirmation is confirmed in turn before its new work starts.
...
The user may mix runtimes across all four supported host-target pairs. On a Claude host, Codex workers use the \`codex-exec\` bridge; on a Codex host, Claude workers use the \`claude-print\` bridge through \`rasen agent dispatch\`. Same-runtime workers remain native only when \`inference\` is absent; an OmniCross-routed same-runtime stage uses the execution view's isolated exec bridge. Pipeline stages may also set \`runtime\`, \`sessionReuse\`, \`sandbox\`, \`model\`, \`effort\`, and credential-free \`inference\`; invocation role flags override runtime defaults for this run because they were forwarded into the final execution-plan command above. The effective **model** for a stage additionally falls through machine config below the pipeline's own role default: stage \`model\` > pipeline \`agents.<role>.model\` > project config \`models.roles.<role>\` > project config \`models.default\` > global config \`models.roles.<role>\` > global config \`models.default\` > the runtime's own default. \`rasen pipeline show <name> --for-execution --json\` already resolves this whole chain and the broker connection identity — read the stage's \`model\`/\`modelSource\`/\`inference\` from there rather than re-deriving it.
