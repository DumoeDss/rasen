...
### 4. Write to the scope-reported project design-docs location

- Run \`rasen context --json\` with the same Store/project/target-line selection and read \`root.scope.paths["project-design-docs"]\`.
- Require that absolute typed location. A Store aggregate or unavailable project scope is an error; do not derive a directory from compatibility root fields, \`planningHome\`, the current directory, or sibling paths.
- Derive a kebab-case slug from the topic — the **verbatim** kebab-case of the description, with NO abbreviation (e.g. "real-time collaboration" → \`real-time-collaboration\`), exactly the way \`rasen-propose\` derives a change name from a description.
- Write one canonical \`<topic-slug>.md\` under the project-design-docs directory. Re-running office-hours for the same topic refines this file in place; separate topics never overwrite each other.
...
- If an active Change exists, record its id and the scope-reported \`changeRoot\` as context inside the document, but do not move the design doc into the Change.
- Inform the user of the exact path. \`rasen-propose\` checks the same typed location for \`<change-name>.md\`.
...
The \`rasen-propose\` command auto-detects this session's design doc and incorporates its insights into the proposal. It reads \`<change-name>.md\` from the same scope-reported project-design-docs location; no Change-root, Store-root, cwd, or sibling-path fallback is used.`;
