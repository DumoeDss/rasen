...
function renderHandoff(
  ...
  rendered = replaceExactlyOnce(
    ...
    `**H.2 Warm-continue guard.** Before ${continuationExamples} of an existing worker (Claude-native \`SendMessage\`, Codex-native \`followup_task\`, Claude bridge exact-session continuation, or Codex bridge thread resume), probe that worker's recorded transcript when one exists. Below its resolved handoff threshold → continue warm. At or above → retire it via a route-appropriate final instruction to write \`<workDir>/handoff/<role>-<n>.md\`, then spawn a fresh successor seeded from that document. Seed from the raw transcript only when the document cannot be produced.\n\n**H.3 Worker self-handoff`,
