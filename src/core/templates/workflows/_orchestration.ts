...
A worker MUST leave its stage's durable artifact before returning — its conversation output alone is NOT a handoff. In particular, the generic expert skills (review / cso / qa / benchmark / design-review), when dispatched, run report-only (see their PREAMBLE "Dispatched vs standalone mode") and write their findings — tagged with canonical severities — to the canonical report file in the **work directory** THEMSELVES (Step F's resolved location; sticky-legacy fallback to the change directory), not to their standalone \`.rasen/*-reports/\` paths: \`review-report.md\` (code review), \`cso-report.md\` (security), \`qa-report.md\` (every QA mode), \`benchmark-report.md\` (performance), \`design-review-report.md\` (design). For a \`qa-report-only\` stage, the dispatch prompt MUST explicitly say to invoke \`rasen-qa\` in report-only/non-UI mode; it is mutually exclusive with the UI \`qa\` stage and still performs browser-based evidence collection. The worker verifies the report is present before returning. State the target report path in the dispatch prompt's stage-specific instructions. These files are what the resume artifact cross-check, \`ship\`'s verification pre-flight, and \`retain\` (report mode) consume.
