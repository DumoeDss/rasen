...
\`status\` is one of pending | in_progress | done | skipped | escalated; a stage counts as complete for resume only when **done | skipped**. (A simpler \`"completed": ["propose","apply"]\` array is also accepted when you are not recording per-stage workers.) Record each dispatched worker's **role**, **agentId**, and **transcript** pointer (Step B). Also record review \`rounds\`, \`openFindings\`, any skips/escalations, per-stage \`handoffs\` and \`strategyAttempts\` (Step H), and the top-level \`sessionHandoff\` when the session itself hands off. \`sessionHandoff.n\` is the session RELAY GENERATION (the example seeds it at \`1\`); Step H.7 caps it at \`maxRelays\`, and a \`sessionHandoff\` record written WITHOUT \`n\` reads as generation 1 and never advances — so always carry \`n\` and increment it each session relay, or the H.7 cap can never trip. Subagent work is otherwise opaque; this record is what lets the run be observed and resumed.
...
A new session has NO live workers — \`SendMessage\` cannot reach a worker spawned in a previous session (**agentIds are dead handles ONLY across a session boundary — a restart**; this scoping matters, because WITHIN a live session \`SendMessage\`-ing a completed or interrupted worker DOES revive the same agent, and that in-session revival IS a transcript-resume — it is the H.4a(b) infra-death and H.4b unticked-\`DONE\` path, not a dead handle). Any request to "resume the worker / resume its session" AFTER an interruption that crossed a session boundary MEANS this ladder: seed a fresh worker from the predecessor's recorded pointers (handoff document, then transcript). Do NOT reverse-engineer the predecessor's progress from artifacts on disk while a pointer exists — artifacts show what survived, but only the ladder carries what the predecessor learned (findings, dead ends, in-flight reasoning). To resume:
...
**H.4b \`DONE\` with unticked tasks is NOT a death.** A \`DONE\` return that left some tasks unticked is an ambiguous completion by a worker that is ALIVE and in-session — not any of the three deaths above. → \`SendMessage\` the SAME worker ("you left 4.4/4.5 unticked — finish them or explain why they're moot"); its reasoning is preserved and **no relay is charged**. Escalate to (c) cold-reconstruct ONLY if that worker is cross-session / unreachable.
