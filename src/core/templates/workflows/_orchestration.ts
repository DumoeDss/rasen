...
1. \`rasen pipeline resume-run <change> <pipeline> --json\` — grants the ready frontier (a review, triage, fix, or re-review phase action) and reports it as \`actions[]\`. This is the RECOVERY seam: it grants what is ready but not yet granted. In a healthy sequential drive the next action was already granted by the previous \`complete\`'s settle pass and arrives on ITS receipt, so \`resume-run\` correctly returns an EMPTY \`actions\` — that means "nothing outstanding", never "the Run is finished". Read \`status\` for the authority.
...
3. \`rasen pipeline complete <change> --run <runId> --from <receipt.json> --json\` — the receipt body is \`{ "completion": <change-run-completion/1>, "uploads": [...] }\` carrying the phase result contract, the actor identity, and the attestation evidence; the \`actionId\` lives INSIDE the completion, not on the command line, and \`--from -\` reads the body from stdin. The receipt returns the actions this settle granted, which is where the next phase action comes from. The Run validates before the Record mutates: a malformed result, a same-actor verification, and a clean claim with open Blocker/Major findings are all rejected rather than absorbed.
