...
**7. Portfolio run-state.** Maintain a parent-level record at \`openspec/changes/<parent>/portfolio-run.json\`: the decomposition plan, child list, dependency DAG (each child's prerequisites), per-child execution mode (serial/parallel) + parallel cohort, per-child pipeline, per-child status, and the current runnable frontier. Each child keeps its OWN per-change \`auto-run.json\`. The portfolio record is AUTHORITATIVE for resume; child-directory/artifact presence is a cross-check. Resume via \`rasen pipeline resume <parent>\` (computes the next runnable child(ren) from the DAG). It also reports \`interruptedChildren\` (were \`in_progress\` at stop — re-engage via warm-seed, do NOT leave stranded) and \`escalatedChildren\` (need human attention). On **partial failure** (a child fails or escalates mid-run): stop that child's dependent chain, leave already-complete independent children intact, and escalate with the open frontier.
