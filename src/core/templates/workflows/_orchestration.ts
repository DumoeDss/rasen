...
- Run \`rasen pipeline show <name> --for-execution --json\` once, appending every run-local role flag supplied to this workflow (for example \`--planner codex --reviewer claude\`), and consume its detected \`hostRuntime\`, \`hostRuntimeSource\`, and per-stage \`runtime\`, \`runtimeSource\`, \`model\`, \`modelSource\`, \`effort\`, \`effortSource\`, \`dispatchMode\`, \`bridge\`, and optional credential-free \`inference\`. These are final resolved values; do not re-derive model, effort, Provider, or broker connection identity. Do not infer the host independently from whichever worker tool happens to be visible.
...
Without \`inference\`, the supported matrix is: Claude host → Claude worker \`native\`; Claude host → Codex worker \`exec-bridge\` with bridge \`codex-exec\`; Codex host → Codex worker \`native\`; Codex host → Claude worker \`exec-bridge\` with bridge \`claude-print\`; unknown host uses \`legacy-fallback\`. On a known host, a stage whose execution view carries OmniCross \`inference\` ALWAYS trusts the view's target \`exec-bridge\` (\`claude-print\` or \`codex-exec\`), including same-host routes, because its per-stage route environment requires an isolated process. A routed stage on an unknown host is unsupported and fails preflight. Never silently substitute a different runtime or a native worker for that route.
...
For a **Codex-native** stage, call \`spawn_agent\` with a concrete leaf task and record the returned **agent id**. Pass the execution view's resolved \`model\` and \`reasoning_effort\` whenever present. If either override is present, set \`fork_turns: "none"\`: a full-history fork inherits the parent agent/model/effort and MUST NOT be described as having switched them. Because a no-history worker starts isolated, its prompt MUST explicitly name the concrete proposal, design, tasks, applicable specs, repository instructions, and other change artifacts it must read. With neither override, the native host default and normal fork behavior remain available. Use \`send_message\` only for intermediate guidance while that worker is running, and \`followup_task\` to start a new turn when an existing worker is idle. The worker's final \`DONE\` or \`HANDOFF\` response is delivered to the LEAD automatically: do NOT send a duplicate completion message. Collect independent work as it arrives. Call \`wait_agent\` only at a true dependency barrier where no useful local or sibling work remains, and prefer one long, barrier-sized event-driven wait over repeated 30- or 60-second polling cycles. Repeated \`Waiting for agents\` loops are orchestration churn, not a required Codex protocol.
...
A worker MUST leave its stage's durable artifact before returning — its conversation output alone is NOT a handoff. In particular, the generic expert skills (review / cso / qa / benchmark / design-review), when dispatched, run report-only (see their PREAMBLE "Dispatched vs standalone mode") and write their findings — tagged with canonical severities — to the canonical report file in the **evidence directory** THEMSELVES (Step F's resolved \`evidenceDir\`, under its sticky-legacy chain), not to their standalone \`.rasen/*-reports/\` paths: \`review-report.md\` (code review), \`cso-report.md\` (security), \`qa-report.md\` (every QA mode), \`benchmark-report.md\` (performance), \`design-review-report.md\` (design). For a \`qa-report-only\` stage, the dispatch prompt MUST explicitly say to invoke \`rasen-qa\` in report-only/non-UI mode; it is mutually exclusive with the UI \`qa\` stage and still performs browser-based evidence collection. The worker verifies the report is present before returning. State the target report path in the dispatch prompt's stage-specific instructions. These files are what the resume artifact cross-check, \`ship\`'s verification pre-flight, and \`retain\` (report mode) consume.

For a stage with non-null OmniCross \`inference\`, the LEAD's single run-state writer MUST copy that exact credential-free route into the stage's \`frozenInference\` field BEFORE its first dispatch. For every fresh or resumed attempt, write a bounded UTF-8 \`rasen.inference/1\` file under \`ephemeraDir\` containing \`{ "schemaVersion":"rasen.inference/1", "route":<frozenInference>, "attempt":{ "runId":<stable run identity>, "stageId":<id>, "attempt":<positive integer>, "sessionId":<exact resume id when applicable> } }\`. Pass that file with \`--inference-file\`; never put the document on argv and never write a lease id, route token, launch descriptor, Admin credential, or child environment to run-state/evidence. On resume, use the \`frozenInference\` returned by \`pipeline resume\`, ignoring later Pipeline/model/endpoint edits, and create a new attempt file so the bridge obtains a replacement lease for the same logical route.
...
rasen agent dispatch --runtime claude --prompt-file <prompt.txt> \\
  ...
  [--model <model>] [--effort <effort>] [--resume <exact-session-id>] \\
  [--inference-file <ephemeraDir/attempt.json>] --json
\`\`\`
...
For a **Codex exec-bridge** stage, compose the complete bounded UTF-8 prompt file and invoke the shipped process bridge:

\`\`\`
rasen agent dispatch --runtime codex --prompt-file <prompt.txt> \\
  --contract <leaf|evaluate> --sandbox <read-only|workspace-write> \\
  --cwd <exact-working-directory> --timeout-ms <bounded-ms> \\
  [--model <resolved-model>] [--effort <resolved-effort>] \\
  [--resume <exact-thread-id>] [--inference-file <ephemeraDir/attempt.json>] --json
\`\`\`

Non-negotiable invariants, not style preferences:

- **Use the shipped bridge rather than a rendered shell command.** It transports the assembled prompt through bounded stdin and closes stdin with EOF, including for Windows native executables and \`.cmd\`/\`.bat\` shims.
...
- **Constrain worker returns with the structured-return contract.** The bridge creates the shared leaf-return schema (or the evaluate-gate schema, for a goal-loop evaluate dispatch) and bounded last-message files and validates them; do not accept free text as a worker's structured result.
...
Accept completion only from the bridge's one \`ok: true\` receipt; the bridge owns JSONL capture, scratch schema/last-message files, strict contract validation, redaction, and teardown. Forward the execution view's resolved model and effort without re-deriving them. Model ids are generic opaque non-empty strings (Luna, Terra, and custom ids use the same path); effort is independently limited to \`low|medium|high|xhigh|max\`. For continuation, invoke the same bridge with the exact recorded \`--resume <threadId>\` and \`--cwd\`. There is no latest-thread lookup. Batch consecutive instructions for one warm thread when none needs an intermediate result; keep one writer per thread and dispatch independent thread ids concurrently.

When you spawn a worker, record its identity in run-state (Step F) FROM THE SPAWN RESULT: Claude-native workers record **runtime=claude**, **dispatchMode=native**, **role**, **agentId**, and **transcript**. Claude exec-bridge workers record **runtime=claude**, **dispatchMode=exec-bridge**, **role**, the receipt's exact **sessionId** and **cwd**, and any surfaced **transcript**/**sandbox**/**model**/**effort** metadata; never invent a native \`agentId\` or Codex \`threadId\`. Codex-native workers record **runtime=codex**, **dispatchMode=native**, **role**, and the returned **agentId**; record a transcript only if the host actually surfaces one, and never invent a \`threadId\` or \`turnId\`. Codex exec-bridge workers record **runtime=codex**, **dispatchMode=exec-bridge**, **role**, and the successful bridge receipt's exact **threadId**, canonical **cwd**, optional **transcript**, **sandbox**, **model**, and **effort**. Codex exec mode yields NO turn id. NEVER record a fabricated \`name\` in place of these handles. Archived records may omit the additive metadata; do not make it mandatory. If a spawn result did not surface a handle, record what you have and flag it for the next dispatch — do not invent one.
...
4. **Record the planner pointer.** Portfolio runs: record the planner's canonical route-specific worker identity at the TOP level of \`portfolio-run.json\` (field \`planner\`) — \`agentId\` plus transcript when surfaced for native dispatch, \`sessionId\` plus \`cwd\` for Claude exec-bridge, or \`threadId\` plus rollout transcript for Codex exec-bridge. Single change: the propose stage's \`worker\` record (Step F) suffices. After a restart, continue or warm-seed the next planner from this pointer per Step F.1 (\`rasen pipeline resume\` reports it).
...
- **Resume.** Re-engage an existing Codex worker through \`rasen agent dispatch --runtime codex --resume <threadId>\` with the exact recorded cwd and the appropriate \`leaf\` or \`evaluate\` contract. The bridge preserves structured result/model/effort flags and closed-stdin transport while omitting Codex's unsupported resume-time sandbox flag. Always resume by explicit \`threadId\` — there is no "latest thread" form (racy under parallel dispatch).
...
**At each quiescent boundary (preview → render → admit):**
1. Run \`rasen pipeline resume-run <change> <pipeline> --json\` (or consume the \`candidates[]\` returned by start/complete/control). Agent candidates are previews only: no Action exists yet. On crash, run resume-run again; an unchanged canonical head reproduces the identical \`candidateId\` and descriptor from the frozen plan. Never render from mutable Pipeline/skill state during retry.
2. For EACH candidate, compose the existing complete role-isolated base prompt from the trusted source workflow instructions, its closed descriptor/structured input, the canonical run-view section, artifact paths, and handoff clauses. Write private ephemera at \`<ephemeraDir>/agent-turn-input.json\` as \`{ "format": "agent-turn-input-manifest/1", "candidates": [{ "candidateId": "<exact preview id>", "prompt": "<complete base prompt>" }] }\`. Include the complete preview frontier exactly once; never put a digest/length in this file, run-state, evidence, or logs.
3. Run \`rasen pipeline admit <change> --run <runId> --turn-input-file <ephemeraDir>/agent-turn-input.json --json\`. Only this command atomically computes turn authority, admits, and grants the Actions. A stale/wrong/missing/extra candidate fails closed; preview again rather than editing an old id. Dispatch ONE role-isolated worker per returned \`actions[]\`, transporting the IDENTICAL base-prompt bytes authenticated by that Action. A **review** or **re-review** action goes to a reviewer that delegates the pass to the \`rasen-review\` engine — do NOT fork or reimplement the review heuristics. A **triage** action classifies the collected findings by severity and disposition. A **fix** action goes to the implementer that wrote the code, or to a separate fixer for a design-level finding. Staffing distinct workers remains YOURS (Step C).
4. \`rasen pipeline complete <change> --run <runId> --from <receipt.json> --json\` — the receipt body is \`{ "completion": <change-run-completion/1>, "uploads": [...] }\`; the \`actionId\` lives INSIDE the completion. Completion commits the result and returns the next \`candidates[]\`; it NEVER auto-admits an agent successor. The Run validates before mutation: malformed result, same-actor verification, and a clean claim with open Blocker/Major findings are rejected.
...
**Read progress from the \`review-cycle\` section**, never from your own accounting: \`rasen pipeline status <change> <pipeline> --json\` returns \`{ round, maxRounds, phase, outcome, findings (each with severity + status), actors, waitReason }\`, and its \`root-dag\` section lists every action with its \`actionId\` and \`deliveryState\` — so \`status\` alone always tells you what is outstanding, whatever a receipt did or did not carry. That section is the ONE source of truth for "where is this loop"; report exactly what it says.
...
**Each round (drive via the reconciler).** At each quiescent boundary follow the SAME preview → complete-prompt render → private \`agent-turn-input-manifest/1\` → \`pipeline admit\` protocol in E.1; resume/start/complete return \`candidates[]\`, and only admit returns \`actions[]\`.
1. For a **work phase** action: dispatch the implementer — warm-reused across ALL rounds (the SAME worker). After the Step H.2 warm-continue guard, continue through the recorded route: Claude-native uses \`SendMessage\` on the same implementer agentId; Codex-native uses \`followup_task\` on the same idle implementer agent; Claude exec-bridge uses \`rasen agent dispatch --runtime claude --resume <sessionId> --cwd <cwd>\`; Codex exec-bridge uses \`rasen agent dispatch --runtime codex --resume <threadId> --cwd <cwd>\`. Exact continuation transports the identical Action-bound base prompt. The implementer NEVER spawns child subagents.
2. For a **judge phase** action: dispatch a **FRESH reviewer worker** (≠ the implementer — author ≠ verifier; the reconciler enforces actor separation at commit time). The reviewer applies the completion-audit discipline (below).
3. \`rasen pipeline complete <change> --run <runId> --from <receipt.json> --json\` commits each phase result and returns the next preview rather than admitting it.
...
- **Ephemera** — run-state (auto-run.json / portfolio-run.json / the goal-loop run artifact), raw logs, captures, and other regenerable intermediates — under \`ephemeraDir\` (\`<executionRoot>/.rasen/changes/<name>/ephemera/\`, in the code checkout this run operates on). Whether it enters Git is the user's \`.gitignore\` decision alone; Rasen writes no ignore rules.
...
\`\`\`json
{
  ...
  "stages": {
    ...
    "apply":   {
      ...
      "frozenInference": { "broker": "omnicross", "runtime": "codex", "upstream": { "kind": "provider", "providerId": "deepseek-api" }, "model": "deepseek-chat", "connection": { "endpoint": "http://127.0.0.1:8765", "controlTokenEnv": "OMNICROSS_ADMIN_TOKEN", "requestTimeoutMs": 5000, "leaseTtlSeconds": 600, "configRevision": "sha256:<digest>" } },
...
\`status\` is pending | in_progress | done | skipped | escalated | delegated; \`delegated\` is parent-stage-only, and **done | skipped | delegated** count as complete for resume. (A simpler \`"completed": ["propose","apply"]\` array is also accepted when you are not recording per-stage workers.) Record each dispatched worker's **role**, **runtime**, **dispatchMode**, and only the identity pointers actually returned by that route (Step B). A routed stage additionally records only its credential-free \`frozenInference\`; lease ids/tokens/descriptors are forbidden. Also record review \`rounds\`, \`openFindings\` (legacy-engine: authoritative; reconciler-engine: a labeled projection of the run view's \`review-cycle\` section, never the thing you branch on), any skips/escalations, per-stage \`handoffs\` and \`strategyAttempts\` (Step H), and the top-level \`sessionHandoff\` when the session itself hands off. Human-facing reports use \`Blocker | Major | Minor | Trivial\`; at the run-state serialization boundary, convert \`openFindings[].severity\` to the lowercase machine enum \`blocker | major | minor | trivial\` exactly as shown above. **autopilot-gate-policy:** record the top-level \`gatePolicy: { effective: 'on'|'off', source: 'flag'|'project'|'global'|'default' }\` ONCE at run start (Step D), and a per-stage \`gateDecision: "auto-approved (<source>)"\` on any stage whose gate was auto-approved rather than confirmed by a human — a human-confirmed gate leaves \`gateDecision\` unset. \`sessionHandoff.n\` is the session RELAY GENERATION (the example seeds it at \`1\`); Step H.7 caps it at \`maxRelays\`, and a \`sessionHandoff\` record written WITHOUT \`n\` reads as generation 1 and never advances — so always carry \`n\` and increment it each session relay, or the H.7 cap can never trip. Subagent work is otherwise opaque; this record is what lets the run be observed and resumed.
...
**1. Produce a decomposition plan.** Define independently-shippable child slices and their dependency DAG. Resolve each pipeline, then create each child with \`rasen new change <semantic-name> --pipeline <childPipeline>\`; this initializes its own \`auto-run.json\` with all stages \`pending\`. Never create a child without run-state or hand-write it later.

**Child names are SEMANTIC kebab-case** — the name says what the slice delivers (a \`<parent>-\` prefix is fine when it aids grouping). Scheduling and DAG-internal identifiers (\`g-003\`-style node ids, slice ids, worker ids) NEVER enter a change name, a probe directory name, or an archive name; they live only in the portfolio run-state's per-child \`node\` metadata, beside the semantic id. Landing paths are named for what they hold, not for how the run was scheduled.
...
**7. Portfolio run-state.** Write the authoritative parent record at \`<ephemeraDir>/portfolio-run.json\` (sticky-legacy: a record that already lives in the legacy work directory or the change directory keeps being written there). Use \`dependsOn\`, never \`prerequisites\`. Child/delivery status is \`pending | in_progress | done | skipped | escalated\`; \`delegated\` is parent-stage-only. Canonical minimum:

\`\`\`json
{
  "parent": "<parent>",
  "children": [
    {"id": "<parent>-<what-this-slice-delivers>", "pipeline": "small-feature", "dependsOn": [], "status": "pending", "node": "g-001"},
    {"id": "<parent>-<what-that-slice-delivers>", "pipeline": "small-feature", "dependsOn": ["<parent>-<what-this-slice-delivers>"], "status": "pending", "node": "g-002"}
  ],
  "delivery": {"status": "pending"}
}
\`\`\`

\`id\` is the semantic change name (it IS the directory name); the optional \`node\` is where a scheduling id lives — never the other way round. Preserve optional planner/mode/cohort/note metadata. Validate immediately with \`rasen pipeline resume <parent> --json\`: a present but invalid portfolio is an error; fix it, never bypass it with parent \`auto-run.json\`. Resume reports runnable, interrupted, escalated, and delivery frontiers. Artifact presence is only a cross-check. On child failure, stop its dependents, preserve independent completions, and escalate the open frontier.

**Issue dispatch — a Store Issue target decomposes into a reviewable plan and STOPS (deliberately NOT this fan-out).** When the dispatch target is a Store Issue (a filed \`rasen/issues/<issue-id>\` record, addressed with \`--store <id>\`), the decompose step does NOT fan out child changes. Produce a decomposition DOCUMENT — a YAML \`nodes:\` list, one \`kind: intent\` node per proposed piece of work, each naming its \`projectId\` and \`targetLineId\` (targets are your PROPOSAL, gated at publication by the planning-member rule — the system never auto-routes work to a project), its \`dependsOn\` edges, an authored \`lifecycle: optional\` where the work is proposed optional (absent reads required; the authored lifecycle compiles ONTO the published intent node — the REVISION, not the document, is the durable record of the required/optional proposal), a \`suggestedPipeline\`, and a \`rationale\` and/or \`uncertainty\`. Publish it as the Issue's next Execution Plan revision with \`rasen store issue plan <issue-id> --from-decomposition <path> --store <id>\` (the document is read-only input and stays byte-identical; every node must be intent-kind and carry its suggestion and at least one of rationale/uncertainty, or publication refuses naming the node and field). Then report the revision REVIEW-READY — its ordinal and node count, readable node by node on \`rasen store issue show <issue-id> --store <id>\` (which also reports the node-level delta of this revision against its predecessor) — and STOP. Do NOT fan out child changes, do NOT create child worktrees, do NOT start any node: revising targets, edges, and lifecycles, and confirming execution, are the HUMAN's flow. This stop-short pause is deliberately distinct from the change-level decompose stage's LEAD-self-audit behavior above (item 2): a CHANGE-level decompose proceeds automatically because its fan-out is the audited plan itself, while an ISSUE dispatch's revision IS the review surface — publishing it and stopping IS the review point. Nothing in this branch alters the change-level fan-out, and a change-level decompose stage never routes here.

**After the human confirms, the dispatch continues through launch contracts.** The human's confirm is \`rasen store issue confirm <issue-id> --store <id>\` (optionally \`--revision <id>\`) over the reviewed revision — after whatever new revisions their review authored through the existing publication channels (a merge or split IS the next revision: a merged node may keep one constituent's nodeId so external references survive; a split mints new nodeIds and re-edges its dependents). Once the operator has confirmed, you MAY continue the dispatch as their agent: for each launchable node the confirm step reported, run \`rasen store issue start <issue-id> --node <id> --store <id>\` and drive the emitted contract's pipeline from its working directory, honoring the dependency frontier so a downstream node waits for completed work (re-confirm nothing already confirmed; a node whose dependencies' work is not complete is not started yet). The operator's confirm is the green light and the revision is the SCOPE: never start a node outside the confirmed revision's wanted work (required and optional only — never cancelled/superseded, never a node the revision dropped), and a revision published AFTER a confirmation is confirmed in turn before its new work starts.
...
6. **Scope guards.** \`reuse.implementer: never\` → always fresh. The design-level **fixer is excluded from reuse** — its value is fresh eyes, so never warm-reuse a prior worker for a fixer role. Under **Tier B**, carry the reuse intent through the transcript warm-seed of Step F.1. Claude exec-bridge uses exact \`sessionId + cwd\`; Codex exec-bridge uses \`threadId\` resume (Step B.2); Codex-native agents use their native agent id and \`followup_task\`, never a fabricated thread id. When a bridge handle is unresumable or context-poor, seed a fresh worker from its handoff/transcript or Codex rollout via warm-seed distillation. Reuse across a user's manually-run sequence of unrelated changes is an explicit NON-goal.
...
- **Quiesce first.** Relay ONLY at a stage boundary: every dispatched worker has returned \`DONE\`/\`HANDOFF\` and run-state is persisted. A probe that fires mid-stage waits for the worker's structured return (H.3 covers the worker's own exhaustion) before the handoff-plus-relay sequence. Additionally, before the relay any **held warm reuse candidate** — a worker that returned \`DONE\` but was RETAINED for a dependent child rather than dismissed (Step G.1) — MUST first write its knowledge digest document — which IS a handoff document: the same rasen-handoff template, written to \`<handoffDir>/<role>-<n>.md\` (Step F's resolved handoff directory, under its sticky-legacy chain) with reason \`retired-between-children\`, so the successor's document-first resume ladder (F.1) finds it — because its cross-change knowledge would otherwise be lost with its session-scoped agent handle.
...
function renderCodexLifecycle(
  ...
  return replaceExactlyOnce(
    source,
    ' and the appropriate \`leaf\` or \`evaluate\` contract.',
    ' and the appropriate \`leaf\` contract.',
...
function renderHandoff(
  ...
  rendered = replaceExactlyOnce(
    ...
    `**H.2 Warm-continue guard.** Before ${continuationExamples} of an existing worker (Claude-native \`SendMessage\`, Codex-native \`followup_task\`, Claude bridge exact-session continuation, or Codex bridge thread resume), probe that worker's recorded transcript when one exists. Below its resolved handoff threshold → continue warm. At or above → retire it via a route-appropriate final instruction to write \`<handoffDir>/<role>-<n>.md\` (Step F's resolved handoff directory, under its sticky-legacy chain), then spawn a fresh successor seeded from that document. Seed from the raw transcript only when the document cannot be produced.\n\n**H.3 Worker self-handoff`,
