...
**H.1 Session pre-flight (auto entry).** Once, at the start of an auto run: \`openspec agent context --latest --json\`. At or above the session threshold, surface a ONE-LINE reminder ("context at <pct>% — consider /opsx:handoff before this run") and proceed on the user's say-so; below it, proceed silently. This is a reminder, not a gate — the user owns session handoff; the harness's auto-compact remains the backstop if they decline.
