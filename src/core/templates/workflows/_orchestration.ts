...
\`status\` is one of pending | in_progress | done | skipped | escalated; a stage counts as complete for resume only when **done | skipped**. (A simpler \`"completed": ["propose","apply"]\` array is also accepted when you are not recording per-stage workers.) Record each dispatched worker's **role**, **agentId**, and **transcript** pointer (Step B). Also record review \`rounds\`, \`openFindings\`, any skips/escalations, per-stage \`handoffs\` and \`strategyAttempts\` (Step H), and the top-level \`sessionHandoff\` when the session itself hands off. **autopilot-gate-policy:** record the top-level \`gatePolicy: { effective: 'on'|'off', source: 'flag'|'project'|'global'|'default' }\` ONCE at run start (Step D), and a per-stage \`gateDecision: "auto-approved (<source>)"\` on any stage whose gate was auto-approved rather than confirmed by a human — a human-confirmed gate leaves \`gateDecision\` unset. \`sessionHandoff.n\` is the session RELAY GENERATION (the example seeds it at \`1\`); Step H.7 caps it at \`maxRelays\`, and a \`sessionHandoff\` record written WITHOUT \`n\` reads as generation 1 and never advances — so always carry \`n\` and increment it each session relay, or the H.7 cap can never trip. Subagent work is otherwise opaque; this record is what lets the run be observed and resumed.

**Write canonical values only (host-runtime neutrality).** \`worker.runtime\` MUST be exactly \`claude\` or \`codex\` — if YOU (the LEAD) are running under neither (e.g. a Codex CLI session driving Claude-role workers, or any other host), OMIT the \`runtime\` field entirely rather than writing a descriptive string like \`"codex-host-fallback"\`; the field means "which of the two known worker runtimes ran this stage", not "what host is the LEAD". Never write JSON \`null\` for an absent optional field (\`transcript\`, \`agentId\`, \`threadId\`, etc.) — OMIT the key instead. A reader tolerates \`null\`/non-canonical \`runtime\` on read (it normalizes them), but the writer contract stays strict, so writing correctly the first time avoids the normalization path entirely.
...
Thresholds and caps resolve from the pipeline's \`handoff\` config, extended with the machine config layers: stage-level \`handoff\` > pipeline \`handoff.roles[<role>]\` (threshold only) > pipeline \`handoff\` > project config \`handoff.roles[<role>]\` (threshold only) > project config \`handoff.threshold\` (threshold only) > global config \`handoff.roles[<role>]\` (threshold only) > global config \`handoff.threshold\` (threshold only) > model preset (the suggested \`handoffThreshold\` of the preset matching the stage's resolved model) > built-in defaults \`{ threshold: 0.5, maxRelays: 3, stallLimit: 2 }\`. Within each machine config scope, a per-role value wins over that scope's scalar threshold; project always wins over global. \`rasen pipeline show <name> --json\` reports each stage's resolved values. Context-heavy roles (reviewer, fixer) typically carry higher thresholds — their bootstrap (diff + specs + findings) is expensive, and retiring them too early buys relays that spend most of their window re-loading. When a role keeps hitting its threshold right after bootstrap, the durable fix is better seeding (hand the successor a distilled context pack), not a higher threshold.
...
- A **mid-task relay** ("should this worker keep going on the task in hand?") compares occupancy to the **handoff** threshold (\`handoff.roles[<role>]\` > \`handoff\` > project config role/scalar > global config role/scalar > model preset > default **0.5**).
...
- **On trigger**: finish or cleanly abort the current atomic step; write \`<workDir>/handoff/<role>-<n>.md\` (Step F's resolved work directory; sticky-legacy fallback: the change directory) per the rasen-handoff template (the eliminated-hypotheses section is MANDATORY for fixer/debugger roles — it is what stops the successor from re-exploring dead ends); return \`HANDOFF { path, reason: compaction|budget|self-assessment, completed: [...], remaining: [...] }\` instead of \`DONE\` — deliver it via \`SendMessage\` to the LEAD, not solely as your final turn's plain-text output, which the LEAD may never observe.
- **On \`DONE\` — durable findings.** The normal \`DONE\` return additionally carries a **durable-findings** clause: 1–3 lines of discoveries that stay true for FUTURE planning (constraints in the code, conventions, gotchas that outlive this task) — not per-task chatter or a status recap. The LEAD relays these findings VERBATIM into the dispatch of the planner that proposes a dependent or subsequent child change (Step B.1), so implementation discoveries feed the next proposal. Every dispatch prompt states this clause so the worker knows to produce it. Deliver \`DONE\` via \`SendMessage\` to the LEAD as well — the same plain-text-is-not-reliably-observed rule applies to both return kinds.
