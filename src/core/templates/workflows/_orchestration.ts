...
**7. Portfolio run-state.** Maintain a parent-level record at \`openspec/changes/<parent>/portfolio-run.json\`: the decomposition plan, child list, dependency DAG (each child's prerequisites), per-child execution mode (serial/parallel) + parallel cohort, per-child pipeline, per-child status, and the current runnable frontier. Each child keeps its OWN per-change \`auto-run.json\`. The portfolio record is AUTHORITATIVE for resume; child-directory/artifact presence is a cross-check. Resume via \`openspec pipeline resume <parent>\` (computes the next runnable child(ren) from the DAG). It also reports \`interruptedChildren\` (were \`in_progress\` at stop — re-engage via warm-seed, do NOT leave stranded) and \`escalatedChildren\` (need human attention). On **partial failure** (a child fails or escalates mid-run): stop that child's dependent chain, leave already-complete independent children intact, and escalate with the open frontier.

### Step G.1 — Cross-child implementer reuse (warm-vs-retire)

A dependent child directly consumes its prerequisite's code, so the implementer that just wrote that code is the warmest possible worker for it — but only when it still has the headroom to take on a whole new change. Between a prerequisite child and its dependent, decide reuse-vs-retire (governed by \`reuse.implementer\`; resolve it and the reuse threshold from \`resolvePipelineReuseConfig(pipeline)\` via \`openspec pipeline show <name> --json\`, default \`auto\` / \`0.25\`). Under \`reuse.implementer: never\`, skip this entirely — always spawn a fresh implementer per child.
...
2. **Probe point = prerequisite review-clean.** Take the reuse decision at the SAME gate that already unblocks the dependent (item 4 of Step G: a dependent MUST NOT begin until every prerequisite is implemented and review-clean), so there is no new synchronization point. Probe the prerequisite implementer's recorded transcript with \`openspec agent context --transcript <path>\` (the Step F worker pointer). Do NOT probe earlier — non-trivial fixes route back to the implementer, so context keeps growing through the review-fix loop; only the review-clean reading is stable.
...
**H.7 Session relay (relaying yourself).** The LEAD can launch its own successor — a verified platform capability (2026-07-07, claude CLI 2.1.202: a session can spawn a new interactive Claude Code window seeded with an initial prompt; the earlier "platform cannot restart the main session" assumption is retired). The mechanics (bootstrap prompt via file indirection or \`-EncodedCommand\` — bare-quoted prompts get truncated by nested shell parsing; platform spawn commands; manual fallback) live in the openspec-handoff skill's "Session relay" section. The orchestration-level invariants:
- **Quiesce first.** Relay ONLY at a stage boundary: every dispatched worker has returned \`DONE\`/\`HANDOFF\` and run-state is persisted. A probe that fires mid-stage waits for the worker's structured return (H.3 covers the worker's own exhaustion) before the handoff-plus-relay sequence. Additionally, before the relay any **held warm reuse candidate** — a worker that returned \`DONE\` but was RETAINED for a dependent child rather than dismissed (Step G.1) — MUST first write its knowledge digest document — which IS a handoff document: the same openspec-handoff template, written to \`openspec/changes/<name>/handoff/<role>-<n>.md\` with reason \`retired-between-children\`, so the successor's document-first resume ladder (F.1) finds it — because its cross-change knowledge would otherwise be lost with its session-scoped agent handle.
