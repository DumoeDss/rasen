...
The mandatory \`--for-execution\` call computes every override's route against the same detected host and rejects an unsupported final route before any worker launches. Supported roles are \`planner\`, \`implementer\`, \`reviewer\`, \`fixer\`, and \`shipper\`. A single run may mix runtimes wherever the route derivation below yields a route.

Routes are DERIVED from the shipped adapters, not read off a fixed table, so never assume a pair is unsupported just because you have not seen it: a host with no dispatch adapter (unidentified, or identified like \`omp\`) resolves \`legacy-fallback\`; a host targeting its own runtime resolves \`native\`; any other dispatch-capable pair resolves \`exec-bridge\` through the bridge the TARGET's adapter declares. Today that derivation yields exactly: Claude host → Claude worker \`native\`; Claude host → Codex worker \`exec-bridge\` with bridge \`codex-exec\`; Codex host → Codex worker \`native\`; Codex host → Claude worker \`exec-bridge\` with bridge \`claude-print\`. Read the route from the \`--for-execution\` output rather than from that list. A \`legacy-fallback\` pair reports \`runtimeSource: legacy-default\` with a named host rather than \`unknown\`: treat it as the legacy compatibility route (Tier C, Step B.3), NOT as a contradiction of the detected host. Never silently substitute a different runtime for an explicit choice.
...
1. **Seed once.** Create/select the parent Change through the Rasen workflow first, then read its absolute \`changeRoot\` from status/instructions and write what YOU already know to \`<changeRoot>/planning-context.md\`: the user's intent verbatim, your codebase findings so far, the decompose plan + dependency rationale, and constraints/decisions already made. The first planner reads this FIRST, then researches only what is missing — not from zero. Never pre-create or guess a Change directory.
...
- **Occupancy.** Probe a Codex worker's context the same way as a Claude worker — \`rasen agent context --transcript <rolloutPath> --json\` — under the SAME thresholds (Step H); a zero-turn rollout legitimately reads 0% occupancy, that is normal, not an error. The converse also holds and matters more: a transcript Rasen has no reader for is an ERROR, not a zero — see the Step H.1b refusal arm. Never treat a reported zero and a refused probe as the same answer.
...
Pass per-change context to a Codex worker by **naming the absolute \`artifactPaths.*.existingOutputPaths\` returned by status/instructions in the dispatch prompt** — this is a verified mechanism, workers genuinely read referenced files, not an aspiration. Reserve repo-root \`AGENTS.md\` for repo-global conventions that apply to every worker regardless of change; it is NOT a per-change context vehicle. Do NOT relocate or \`cd\` a worker into a change directory to trigger nested \`AGENTS.md\` auto-discovery as a substitute for naming files explicitly.
...
**Per-class blackboard.** Each artifact lands in its class's directory, always at the ABSOLUTE path the payload reports — never at a guessed cwd-relative Change path, or a Store-selected run strands it where a resumer cannot find it:
...
Agents cannot feel their own context usage; they MEASURE it. \`rasen agent context\` reads exact occupancy from a transcript's recorded API usage — \`--latest\` probes your own (the LEAD's) main session, \`--transcript <path>\` probes a worker via the pointer recorded in run-state (Step B). The two flags fail DIFFERENTLY and both arms are mandatory: \`--latest\` degrades softly (H.1 unavailable arm), \`--transcript\` refuses hard (H.1b refusal arm). Probe ONLY at the discrete decision points below. NEVER inject a running token countdown into any agent's context — it breaks the prompt-cache prefix and induces premature wrap-up (context anxiety).
...
**H.1 unavailable arm (mandatory).** The pre-flight can legitimately answer \`{"available": false, "reason": "no-transcript"|"unsupported-host", "detail": "..."}\` at exit 0, with NO \`contextTokens\`/\`limit\`/\`pct\`/\`remainingTokens\` — \`unsupported-host\` means the harness this session runs in has no context-probe adapter, so no reading exists to take. An Oh My Pi session is NOT that case: it has its own context reader, so an implicit \`--latest\` there reports THAT session’s own occupancy — treat a refusal from an Oh My Pi host as a bug, not as normal. Branch on \`available\`, never on the presence of \`pct\`. On \`available: false\`: record it if you track your own state (e.g. \`unavailable-<runtime>\`), proceed exactly as if the threshold had not been reached, and do NOT offer the three-way choice, retry, force \`RASEN_AGENT_RUNTIME\`, or treat the shape as an error. Do NOT infer "below threshold" from a missing \`pct\` — an unmeasured session is unmeasured, not empty. This arm applies to every \`--latest\` pre-flight in this playbook and in the workflow that invoked it.

**H.1b \`--transcript\` refusal arm (mandatory).** Unlike \`--latest\`, an explicitly named \`--transcript <path>\` is an INPUT, so Rasen refuses it rather than degrading: when the file belongs to a harness Rasen ships no context reader for, the command exits NON-ZERO with a message naming that harness ("No context reader exists for the recognized session runtime ..."). Claude, Codex and Oh My Pi session files all HAVE readers, so a named file of any of those three is measured, not refused. No \`contextTokens\`/\`limit\`/\`pct\`/\`remainingTokens\` is printed. This is a correct, deliberate refusal — the alternative would be reading one harness's file with another harness's field names and reporting a confident number that describes nothing. It is NOT a crash, NOT format drift, and NOT a reason to file feedback. Everywhere this playbook makes a \`--transcript\` probe a gate, a refusal is a THIRD outcome beside below-threshold and at-or-above: treat that worker's occupancy as UNMEASURED — do not warm-continue on the strength of an unmeasured probe, and do not retire on it either. Retire only if you have another reason to; otherwise proceed as if no probe were available and say so. Re-forcing the read with \`--runtime\` is prohibited: it reinstates exactly the wrong number the refusal exists to prevent.
