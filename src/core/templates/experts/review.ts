...
import { PREAMBLE, BASE_BRANCH_DETECT, DESIGN_REVIEW_LITE, TEST_COVERAGE_AUDIT_REVIEW, ADVERSARIAL_STEP } from './_shared.js';
...
- **Spec source.** If this branch has an associated Rasen Change, identify its id from explicit context or ask the user, then run \`rasen status --change <change-id> --json\` with the complete Store/project/target-line selection. Read proposal and tasks only from their \`artifactPaths.*.existingOutputPaths\`; never derive the Change directory from the branch, cwd, or a Store/repo root. If no Rasen Change can be identified, skip the Spec axis and note "no spec available".
