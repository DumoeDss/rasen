...
import { DEFAULT_HANDOFF_CONFIG } from './pipeline-registry/types.js';
...
export interface HandoffThresholdReport {
  ...
  /** True when `pct` (measured occupancy) meets or exceeds `threshold`. */
 ...
 * default (0.5), and reports whether `pct` meets or exceeds it. Role-agnostic
 * by design — a transcript probe has no stage identity, so pipeline/stage/role
 * overrides (which apply only to `resolveStageHandoffConfig`) do not apply
 * here. Shares `resolveHandoffThresholdLayers()`
 ...
 * exit code.
 */
export function resolveHandoffThresholdReport(pct: number, cwd: string = process.cwd()): HandoffThresholdReport {
  ...
  return { threshold, thresholdSource, shouldHandoff: pct >= threshold };
}
