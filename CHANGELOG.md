...
- **Explicit project-knowledge bundle export** — `rasen knowledge bundle export --project <projectId|root> --to <path> [--json]` creates one strict, versioned portable file containing only the project's own canonical learned records. It refuses an occupied destination and any record containing a machine path, and leaves the catalog, checkout, and registrations byte-identical. This child does **not** include Store transport, bundle import, machine-preparation integration, or portable run checkpoints.
