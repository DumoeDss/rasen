...
- Made run-state writing crash-safe (temp file plus rename), and made a knowledge-context update inject only that field — a LEAD's own hand-written progress and handoff records are left byte-for-byte as written rather than rewritten through a parse/serialize round trip.
