...
- **Behavior change — `autopilot.gates` is now a mask base, not a blanket switch.** Per-stage gate configuration (`pipelines.<name>.gates.<stage>`, resolving project > store > global) now outranks it: a per-stage `on` pierces an `autopilot.gates: off` base for that one stage, and a per-stage `off` silences one stage under an `on` base. `autopilot.gates: on` (the default) still means "honour the stage definitions" exactly as before, and the always-pausing `gate: 'vet'` carve-out is unchanged. Existing `autopilot.gates: off` configurations keep auto-approving every ordinary gate unless a per-stage `on` is set. The LEAD now reads each stage's effective gate from `rasen pipeline show <name> --json` (which reports the masked `effectiveGate` and its `gateSource`) rather than combining layers itself.
