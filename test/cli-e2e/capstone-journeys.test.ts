...
describe('capstone persona journeys (6.1)', () => {
  ...
  it('journey 3 — externalized planning: pointer repo runs the lifecycle without --store', async () => {
    ...
    // Finalization is live, and its first gate is the outcome: a Store v2
    // Change ends in exactly one explicitly declared terminal state, with no
    // default and nothing inferred from the work looking finished. The refusal
    // precedes any access, so the Change directory is still there afterwards.
