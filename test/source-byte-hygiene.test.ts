...
const EXCEPTIONS: readonly Exception[] = Object.freeze([
  ...
  // (The two 0.1.7-era entries that were here — run-state.test.ts
  // replacement-char and skill-templates-parity.test.ts bom — came out on
  // this line: those files are clean here, and the staleness half requires a
  // clean file's entry to be deleted.)
]);
