...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `rasen-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `rasen-migration-config-${randomUUID()}`);
  ...
  it('ignores unknown custom skill and command files when scanning workflows', async () => {
    ...
    const customCommandPath = path.join(projectDir, '.claude', 'commands', 'rasen', 'my-custom.md');
