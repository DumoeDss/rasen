...
describe('atomic workspace coordination recovery', () => {
  ...
  it('claim content digest is exactly sha256(content) hex, over the raw bytes and nothing else', async () => {
    // atomicWorkspaceDigest (dependencies.ts) is not exported, so this file's
    // own `digest()` helper above is a hand-written duplicate of the formula,
    // not a call into production code: it would silently track a change to
    // the real encoding (e.g. hex -> base64) rather than catch one, because
    // both sides would drift together (the digest is embedded in the claim
    // filename too, so a drifting duplicate would just fail to *locate* the
    // claim file rather than exercise this assertion). The claim file is
    // located here by scanning for whatever `*.claim` file production
    // actually wrote, independent of this file's own digest formula, so a
    // regression in the real encoding lands on the `contentDigest` value
    // check below rather than on a filename mismatch upstream of it.
    const target = temporaryTarget();
    failDirectorySyncOnce(path.dirname(target));
    await expect(atomicWorkspaceWriteText(target, intended)).rejects.toMatchObject({
      code: 'EIO',
    ...
    vi.restoreAllMocks();
    const directory = path.dirname(target);
    const claimFileName = fs.readdirSync(directory).find(name => name.endsWith('.claim'));
    expect(claimFileName).toBeDefined();
    const claim = JSON.parse(
      fs.readFileSync(path.join(directory, claimFileName as string), 'utf8')
    ) as { contentDigest: string };
    expect(claim.contentDigest).toBe(
      '3b9f1530860219243b654d520de495dd6a77dcdcab528c111a45b882388dd70c'
    );
  });
});
