...
describe('a query takes no lock and completes while writers hold locks', () => {
  ...
  it('completes a listChanges query while the issue and scope locks are held', async () => {
    ...
    const issueKey = issueLockKey({
      storeUid: f.storeUid,
      issueUid: parseIssueUid('11111111-1111-4111-8111-111111111111'),
    });
    const scopeKey = scopeLockKey({ storeUid: f.storeUid, projectId: PROJECT, targetLineId: LINE });
  ...
  it('completes a listIssues query while the issue lock is held by another Issue', async () => {
    ...
    const coordination = createNodeWorkspaceCoordination(f.globalDataDir);
    const otherIssueKey = issueLockKey({
      storeUid: f.storeUid,
      issueUid: parseIssueUid('22222222-2222-4222-8222-222222222222'),
    });
    ...
    await withIssueLock(coordination, otherIssueKey, async () => {
      ...
      expect(result.issues[0].issueId).toBe(created.identity.uid);
    });
  });
});
...
describe('baseline inventory and invariants (1.1, 1.2)', () => {
  it('the Issue layout addresses are Store-level: no project or target line in the address', () => {
    ...
    const issueDir = resolveStorePlanningLayoutV2Path(
      root, { kind: 'issue', issueStorageKey: parseIssueStorageKey('x') }, 'posix'
    );
    const issueRecord = resolveStorePlanningLayoutV2Path(
      root, { kind: 'issue-record', issueStorageKey: parseIssueStorageKey('x') }, 'posix'
    );
    const plansDir = resolveStorePlanningLayoutV2Path(
      root, { kind: 'execution-plans', issueStorageKey: parseIssueStorageKey('x') }, 'posix'
    );
    const revision = resolveStorePlanningLayoutV2Path(
      root, { kind: 'execution-plan', issueStorageKey: parseIssueStorageKey('x'), revisionId: '0001' }, 'posix'
    );
  ...
  it('every Issue address input takes no projectId and no targetLineId', () => {
    ...
    const root = '/srv/store';
    const a = resolveStorePlanningLayoutV2Path(root, { kind: 'issue-record', issueStorageKey: parseIssueStorageKey('x') }, 'posix');
    const b = resolveStorePlanningLayoutV2Path(root, { kind: 'issue-record', issueStorageKey: parseIssueStorageKey('x') }, 'posix');
