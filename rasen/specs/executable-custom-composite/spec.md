# executable-custom-composite Specification

## Purpose
Let a user author their own Composite declarations and have them execute with exactly the
semantics the built-ins get: CompositeRef inlined into the runtime plan, BoundedLoop over
a composite body, full Canvas authoring (create, reference, fold/expand, edit scalar
fields, edit body stages and their connections, delete under a reference guard), static
validation that rejects invalid custom-authored shapes, isomorphic plans for built-in and
custom definitions, an export/import round-trip that preserves the semantic digest,
composite drill-down projection, and recovery at composite body stage boundaries.
