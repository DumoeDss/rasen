...
## Purpose
Define `rasen schema validate` behavior for validating schema syntax, structure, templates, and dependency graphs.

## Requirements
### Requirement: Schema validate checks schema structure
The CLI SHALL provide a `rasen schema validate [name]` command that validates schema configuration and reports errors.

#### Scenario: Validate specific schema
- **WHEN** user runs `rasen schema validate my-workflow`
...
- **WHEN** user runs `rasen schema validate` without a name
- **THEN** system validates all schemas in `rasen/schemas/`
- **AND** displays results for each schema
- **AND** exits with non-zero code if any schema is invalid

#### Scenario: Schema not found
- **WHEN** user runs `rasen schema validate nonexistent`
...
- **WHEN** user runs `rasen schema validate my-workflow` and `schema.yaml` has syntax errors
...
- **WHEN** user runs `rasen schema validate my-workflow --verbose`
