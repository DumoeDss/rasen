# catalog-backup-cleanup-safety Specification

## Purpose
When a learned-skill catalog rewrite publishes a new record, a failure during backup cleanup MUST NOT destroy the new record or restore an already-partially-deleted backup. The new record is retained, inert debris is left for self-healing, and a degraded status is reported.
## Requirements
