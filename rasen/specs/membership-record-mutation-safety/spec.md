# membership-record-mutation-safety Specification

## Purpose
Read-modify-write and delete operations on a project membership record (eject, migrate-membership) MUST take the same shared lock as add-project and re-read inside it, so a concurrent role addition is never silently overwritten or dropped.
## Requirements
