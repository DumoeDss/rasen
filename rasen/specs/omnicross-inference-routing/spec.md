# omnicross-inference-routing Specification

## Purpose
OmniCross Inference Routing governs how a pipeline stage binds its already-resolved runtime and model to a brokered upstream through a local OmniCross daemon: resolving loopback-only connection configuration and an environment-held control credential, freezing the credential-free logical route into execution records, acquiring and validating a Route Lease that encloses exactly one agent attempt, reducing the lease response to a constrained Claude or Codex runtime binding, supervising renewal, cancellation, and release, and failing closed with stable redacted errors — while re-leasing the same logical route on resume and leaving concurrent and non-OmniCross dispatches isolated and unchanged.
