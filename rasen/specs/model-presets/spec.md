# model-presets Specification

## Purpose
Defines a built-in model-preset registry keyed by model-id substring patterns, carrying each family's context-window size and optional suggested handoff/reuse thresholds. It is the single source of context-window sizes for `rasen agent context` and supplies a model-aware layer in handoff/reuse threshold resolution, letting large-window model families run with fewer relays without per-pipeline configuration.
