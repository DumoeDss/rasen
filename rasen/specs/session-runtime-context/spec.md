...
Before a Store session begins working on a project, the system SHALL confirm that the Store resolves and is healthy, that the chosen checkout exists on this machine, that the checkout's own recorded identity is the project that was chosen, and that the Store vouches for that project. The Store SHALL be taken to vouch for the project when **either** authority does: the Store's own membership record for that project, **or** the project's own durable Store declaration resolving to this Store. The two are a union and never a narrowing — a project linked to the Store only by its own declaration, which is the linkage that predates membership records, SHALL remain selectable, and a project the Store records SHALL remain selectable whatever its own declaration says. The declaration arm SHALL be decided by resolving the declaration through the shared resolver and comparing the resolved Store root, never by comparing a declared display name, since a durable declaration carries only a permanent identity. A project whose own default planning Store is a different Store SHALL remain a valid choice, because the session records its planning Store explicitly. A failure at any step SHALL prevent the session from starting and SHALL name which check failed and the command that repairs it.

#### Scenario: A valid choice starts the session

- **WHEN** a user chooses a project that the Store's membership permits, whose checkout exists and carries that project's identity
...
- **WHEN** the chosen project has no membership record in the Store but its own durable declaration resolves to that Store
- **THEN** the session starts with that project as its execution target
- **AND** the choice is accepted on the resolved Store root, so a second Store sharing the same display name is not mistaken for it
...
- **WHEN** neither the Store's membership record nor the chosen project's own declaration names this Store
- **THEN** the session does not start and the failure names the missing membership and the command that adds it
- **AND** a declaration that names a different Store, or one that cannot be resolved on this machine, does not vouch for the project

#### Scenario: A project that plans elsewhere is still a valid choice

- **WHEN** the chosen project's own default planning Store is a different Store from the one the session plans in
