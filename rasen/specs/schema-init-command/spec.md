...
## Purpose
Define `rasen schema init` behavior for creating project-local schema skeletons in interactive and non-interactive modes.

## Requirements
### Requirement: Schema init command creates project-local schema
The CLI SHALL provide a `rasen schema init <name>` command that creates a new schema directory under `rasen/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `rasen schema init my-workflow`
- **THEN** system creates directory `rasen/schemas/my-workflow/`
...
- **WHEN** user runs `rasen schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `rasen schema init existing-schema` and `rasen/schemas/existing-schema/` already exists
...
- **WHEN** user runs `rasen schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `rasen schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `rasen schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates `rasen/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Set as default via flag
- **WHEN** user runs `rasen schema init my-workflow --default`
- **THEN** system creates schema and updates `rasen/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Skip setting default
- **WHEN** user runs `rasen schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `rasen/config.yaml`
...
- **WHEN** user runs `rasen schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `rasen schema init "invalid name" --json`
