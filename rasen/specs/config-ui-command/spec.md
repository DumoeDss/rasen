...
This spec defines the `rasen config ui` subcommand, which starts the localhost config API server, resolves the launch project, opens the user's browser to a token-carrying URL, and serves the optional configuration UI package. It governs how the command handles browser launch, optional UI package resolution and static serving, and clean shutdown so process exit is never blocked by open sockets.
