...
## Purpose
Define `rasen change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `rasen change show update-error --json`
...
- **WHEN** executing `rasen change list`
- **THEN** scan the rasen/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `rasen change show update-error --requirements-only`
...
- **WHEN** executing `rasen change validate update-error`
...
- **WHEN** executing `rasen list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'rasen list' is deprecated. Use 'rasen change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `rasen list --all`
...
- **WHEN** executing `rasen change show` without arguments
...
- **WHEN** executing `rasen change show` without a change name
...
- **WHEN** executing `rasen change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `rasen change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
