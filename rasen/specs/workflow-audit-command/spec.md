...
## Purpose
Defines the `audit` workflow — skill `rasen-audit`, invoked as `/rasen-audit` — that guides a user through auditing a Claude Code or Codex CLI session's token spend: finding the session id, running `rasen agent audit`, and interpreting the resulting report in the vocabulary appropriate to that session's runtime. It is part of ALL_WORKFLOWS but not CORE_WORKFLOWS, since session cost auditing is an optional diagnostic reached for on demand rather than part of the minimal first-run set.
