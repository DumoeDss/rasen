# store-obtain-identity-verification Specification

## Purpose
When obtaining a Store, the cloned Store's identity is verified against the declaration before it is published or registered. An alias-only obtain with no UID falls back to the declared alias/ID; any mismatch fails closed with zero writes.
## Requirements
