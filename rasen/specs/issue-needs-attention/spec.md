# issue-needs-attention Specification

## Purpose
This capability answers the portfolio operator's question `what across my Issues needs a human right now` with one deterministic attention derivation per Issue: exactly the items a human must act on, drawn from one closed vocabulary — `failure`, `blocked-behind`, `waiting-human`, `acceptance-awaiting`, and `problem` — computed from the status projection's own facts alone, persisted nowhere, identical over unchanged evidence. Ordinary serial waits and healthy in-flight work are honestly absent from the items rather than noise, a failure among running siblings reads unmasked, and every item carries its Issue's identifier, the node it names where it names one, and the Issue's phase and health beside the fact, so an item never needs a second read to explain its context. The `rasen store attention` scan verb aggregates the items across Issues in fail-first order with every scanned Issue's phase, health, and attention count visible in the summary and an honest empty state when nothing needs attention; it writes nothing.
