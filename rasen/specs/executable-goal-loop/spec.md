# executable-goal-loop Specification

## Purpose
Make the goal loop — work→judge, bounded by `maxRounds` — a reconciler-executed
bounded-loop rather than a prompt-owned state machine: goal-cycle node execution, three
Zod-validated domain result contracts, structural work/judge actor separation, an explicit
exhausted outcome at the round cap, a `goal/1` projection section, deterministic restart
at quiescent boundaries, all three goal-loop built-ins migrated onto canonical Runs, a
measure-gate failure that cannot deadlock the Run, and `goal-run.json` demoted to a
compatibility projection derived from the Record.

`rasen-goal` is a thin launcher over this capability.
