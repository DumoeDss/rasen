# issue-ready-set-scheduling Specification

## Purpose
This capability answers, for an Issue's latest published Execution Plan revision, the single scheduling question "what may run now, and why may nothing else" with one deterministic ready set: exactly the Change nodes the plan still wants whose observed execution has not started and whose every dependency's observed work is complete on the work-complete basis the status projection already enforces, cross-project dependencies included. The set derives on read through the status projection's own node facts alone (persisted nowhere, identical over unchanged evidence) and is shared verbatim by the `rasen store issue ready` read verb, the `store issue start` frontier, and the confirm launch scope, so the three surfaces cannot drift apart. Every node outside the set is named with a closed reason vocabulary rather than silently dropped. Work delivered and archived under a legacy record basis (no v2 outcome to read) counts as complete for scheduling, releasing its dependents without hand-mirrored run-state, while an archive record that exists in v2 shape but fails validation fails closed as unknown; an Issue whose latest revision does not read back yields no ready set rather than an empty one, because "no readable plan" and "nothing runnable" are different truths.
