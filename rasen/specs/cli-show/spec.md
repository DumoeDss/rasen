...
## Purpose
Define top-level `rasen show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `rasen show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `rasen show <item>` or `rasen change/spec show`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `rasen change show`/`rasen spec show`
...
- **WHEN** executing `rasen show <item> --json`
...
- **WHEN** showing a change with `rasen show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `rasen show <spec-id> --requirements`
