# store-layout-migration Specification

## Purpose
Define the user-facing contract of `rasen store migrate-layout`, which moves a legacy flat
Store to the partitioned layout v2: evidence-only attribution that blocks apply whenever an
item's ownership is unknown, conflicting, non-member, or shared; explicit mapping-file
resolution that can never contradict recorded identity; staged, digest-verified publication
that is resumable and rollbackable, with the layout declaration flipped last and flat-tree
retirement as a separate idempotent step; complete reporting of every other ref still
carrying the flat layout; a defined, non-dead-end outcome for an empty legacy flat Store;
and refusals that name the item, the reason, and a repair that actually works.

The contract exists because the flow shipped with no spec at all: it was implemented against
design SS11 and exercised only by fixtures that seeded their own shapes, so its fail-closed
attribution, shared-spec provenance, recoverable publication, and refusal-legibility
guarantees were recorded nowhere the archive process protects.
