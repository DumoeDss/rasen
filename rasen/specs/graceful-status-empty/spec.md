...
## Purpose
Make `rasen status` exit gracefully when no changes exist while preserving existing status validation behavior.
...
- **WHEN** user runs `rasen status` without `--change` and no change directories exist under `rasen/changes/`
- **THEN** the CLI prints `No active changes. Create one with: rasen new change <name>` to stdout and exits with code 0

#### Scenario: No changes exist, JSON mode
- **WHEN** user runs `rasen status --json` without `--change` and no change directories exist
...
- **WHEN** user runs `rasen status` without `--change` and one or more change directories exist
...
- **WHEN** user runs `rasen status --change non-existent`
- **THEN** the CLI throws an error with message `Change 'non-existent' not found`

#### Scenario: Other commands unaffected
- **WHEN** user runs `rasen show` or `rasen instructions` without `--change` and no changes exist
