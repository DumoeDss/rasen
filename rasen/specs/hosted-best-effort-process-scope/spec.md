# hosted-best-effort-process-scope Specification

## Purpose

Give every hosted-session process scope on darwin, linux, and win32 the same honest contract: the scope declares `exactCancel: false` and `scopeEmptyProof: false` before the workload starts, and every terminal state it produces uses the declared-unproven vocabulary rather than claiming more than the platform can prove. darwin and linux share one POSIX best-effort implementation (group signals, a bounded grace period keyed on whole-group emptiness, one bounded final observation); win32 keeps the legacy capsule's Job-object kill mechanics and its `KILL_ON_JOB_CLOSE` daemon-death teardown property, but speaks only the declared-unproven vocabulary at the hosted seam. Transport or controller loss is retained as typed uncertainty and never authorises release, and acceptance rests on real production-path receipts with demonstrated failing counterparts rather than kernel-enforced proof.
## Requirements
