...
## Purpose
Define `rasen schema which` behavior for reporting resolved schema source, location, and fallback details.

## Requirements
### Requirement: Schema which shows resolution result
The CLI SHALL provide a `rasen schema which <name>` command that displays where a schema resolves from.

#### Scenario: Schema resolves from project
- **WHEN** user runs `rasen schema which my-workflow` and schema exists in `rasen/schemas/my-workflow/`
...
- **WHEN** user runs `rasen schema which my-workflow` and schema exists only in user data directory
...
- **WHEN** user runs `rasen schema which spec-driven` and both project and package have `spec-driven`
...
- **WHEN** user runs `rasen schema which spec-driven --json`
- **THEN** system outputs JSON with `name`, `source`, and `path` fields

#### Scenario: JSON output with shadows
- **WHEN** user runs `rasen schema which spec-driven --json` and schema has shadows
...
- **WHEN** user runs `rasen schema which --all`
...
- **WHEN** user runs `rasen schema which --all --json`
