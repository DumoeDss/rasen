...
## Purpose
Read-only aggregate query over a Store's planning state: Issues, Execution Plans, project
memberships, and per-project/per-target-line Changes. The query is the single read surface
that crosses project partitions and target lines without granting authority to mutate any of
them.
## Requirements
