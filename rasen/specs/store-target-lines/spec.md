...
## Purpose
Records stable target-line identities in a Store so that a version development line (e.g.
0.1.7, 0.2.0) has a portable name, a code ref, and a digest — independent of branch names
or checkouts. Target lines are the scope dimension that separates parallel development lines
and the anchor a workspace pair binds to.
## Requirements
