...
### Decision 13. The runtime renders and admits the consultation Teacher turn itself

Decision 12 exists because the runtime cannot reproduce a workflow stage's prompt: role instructions, workflow/skill text, artifact paths, handoff clauses, and task context live in the source LEAD. That premise does not hold for the consultation Teacher. Its turn input is the closed `teacher-consultation/invocation/1` envelope, which reconciliation derives from the attested `CONSULT` step and the frozen binding with no mutable Pipeline or skill content, and which the consultation driver already transports as `canonicalJson(action.agent.input)`.

Routing the Teacher through preview → manifest → `admit` was rejected. `ecp-consultation-runtime` requires ECP to "directly admit the bound Teacher Action" and states the consultation data path "SHALL NOT require a LEAD Action or LEAD-authored relay"; a turn-input manifest is exactly such a relay. It would also make the LEAD the author of the Teacher's question bytes, contradicting "Worker output and driver input SHALL NOT select or replace the Teacher authority" and creating a substitution hazard that does not exist today. Answering the question by exempting the Teacher from turn-input authority was likewise rejected: it would put a hole in the M4 invariant that no new agent Action executes unbound bytes.

So the runtime is the trusted renderer for this turn. It renders the exact dispatch bytes at admission, binds them under the same `agent-turn-input/1` authority, and labels them `rasen.runtime-derived-consultation-turn/1`. The distinct label matters because recording these bytes as `rasen.driver-rendered-turn/1` would assert in the Record that a driver authored something it never saw. The label participates in canonical Action bytes (so a relabelled Action fails complete receipt equality) but is deliberately kept out of the digest preimage, so transport authentication stays exact length + digest for every contract and no historical Action is invalidated.

Two honest limits. First, for the Teacher the digest is tautological — a verifier can recompute it from the committed `agent.input`. The executable authority actually comes from `agent.input` being covered by complete canonical Action equality; the binding exists to keep one uniform invariant rather than to add proof. This is not the `JSON.stringify(agent.input)` backfill that task 7.5 forbids: that prohibition targets the executor inventing authority at dispatch for an Action that has none, whereas here the runtime is the contemporaneous author at admission. Second, the continuation path needs no change: `dispatchGrantedContinuation` already chains `grant.inputDigest` to a Record-committed `requestId`, which is the same property by a different mechanism, and confirms this shape is the established one for runtime-owned turns.

Explicit admission refuses a manifest that reaches a Teacher candidate rather than accepting rendered bytes it would then discard. That refusal is reachable: when the sponsored Teacher read conflicts with a workspace reservation, the consultation commits as `requested` with the Teacher candidate still on the frontier.

### Decision 14. Only the Teacher's turn is server-derived at the daemon face

Decision 13 makes the runtime the author of the *Teacher* turn. The daemon face previously applied the same server-side derivation to **every** consultation-driven Action, keyed on `participatesInConsultation()`, which is true for the eligible *source* implementer as well. Combined with Decision 11's transport authentication that is fatal: the source's committed binding holds the LEAD's real driver-rendered prompt, the face substitutes `canonicalJson(agent.input)`, the executor compares the substitution against the binding, and the dispatch dies with `execution_input_mismatch` before any backend. No `CONSULT` step can be produced, so the shipped `ecp-consultation-runtime` lifecycle is unreachable in production. The substitution predates this change and the authentication belongs to it; neither is wrong alone, and the collision only exists once both are present.

The face therefore **stops deriving server-side**. Every Action that reaches the dispatch call keeps its caller-transported bytes, which its own turn-input binding authenticates. The repair is the removal of the `consultationDriven` condition, not the addition of a narrower one.

Be precise about what protects the Teacher, because the narrower condition reads as if it does and does not. A granted Teacher never reaches that ternary: it is refused ~95 lines earlier with `legacy_teacher_dispatch_forbidden`, so the Teacher arm is provably unreachable and the face's effective behaviour is `envelope.turnInput` unconditionally. The Teacher's bytes stay server-derived because `executeOnce` in `createManagementExactTeacherAttemptModule` constructs `canonicalJson(action.agent.input)` on the exact-attempt path, and because that earlier refusal keeps the legacy path closed. The remaining arm is a deliberate backstop for the day that refusal is relaxed, and is labelled as such in the code; it is not today's enforcement. Trusting `envelope.turnInput` for a *Teacher* would still be wrong — it would hand the bound Teacher's question to the caller and undo Decision 13 — which is why the backstop stays rather than being deleted.

One consequence is easy to miss. A consultation-eligible **source** Action committed before `agent.turnInput` existed has no binding, so the executor takes its documented historical-unrouted path: no authority check, and the caller's transported bytes execute verbatim. Before this repair the face's derivation masked that by overwriting caller text for every consultation-driven Action — which means the old behaviour was overriding the executor requirement that a historical unbound Action "SHALL retain its prior caller-rendered turn behavior", not implementing it. The fallback is therefore intended, but it is a third enforcement mode alongside digest comparison (bound Actions) and server-side construction (Teacher), and it applies only to Records predating the binding — on an unreleased 0.2.0, dev-local Records rather than shipped installations.

The defect survived a green suite because the consultation fixture rendered `JSON.stringify(candidate.input)`, which for these inputs is byte-identical to the substitution — a fixture that behaves exactly like the bug. The fixture now renders a realistic prose base prompt, and the fake backend and its assertions no longer assume a turn input is JSON: a prose turn is the source's, a JSON envelope is the Teacher's or a continuation's, which is the production distinction. With that in place, restoring the old keying turns three real-HTTP journeys red.
