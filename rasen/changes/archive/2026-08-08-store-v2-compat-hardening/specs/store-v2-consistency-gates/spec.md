...
Keep a Store coherent after the tools stop watching. Layout v2's partitions, identities, target lines, and outcomes are all facts committed to a Git repository that a person can edit, merge, and rebase without Rasen's involvement, so every invariant the writing side enforces has to be re-checkable from the read side. This capability is that read side: one shared read-only diagnosis behind every command that reports a Store's health, so two doctors can never disagree; a cross-check of each committed Change and Archive entry against the target-line catalogs, which names both sides of a disagreement and repairs neither; a source census that keeps every reader of a layout-versioned record on the layout dispatcher, enumerated per site rather than relaxed into a pattern; and the demonstrated property that a standalone project and an unmigrated Store behave exactly as they did before any of this existed. Every surface here reports. None repairs, none synthesizes a fact its evidence does not carry, and none replays a spec delta.
