...
**Reason**: The `rasen:` colon namespace belonged to the retired commands era (removed by the skills-only delivery change). A colon-form `name` in generated SKILL.md frontmatter makes Claude Code's slash-completion popup display `/rasen:<x>` while the actual invokable identifier is the hyphen directory name — a permanent visible mismatch.

**Migration**: Skill names unify to the hyphen form: every skill's `name` equals its `rasen-<x>` directory name (see the added requirement "Skill names use the rasen- hyphen form and match the skill directory"). Pre-existing colon references in user-authored pipelines and workflow packages resolve through the legacy skill-id mapping.
