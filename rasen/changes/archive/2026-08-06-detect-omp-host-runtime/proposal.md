...
- Leaves the `RASEN_AGENT_RUNTIME` override half-applied to the context probe, as a known gap: the override selects the host (so it lifts the new refusal) but never selects the transcript store, which still comes from `--runtime` alone. `RASEN_AGENT_RUNTIME=codex` plus an implicit `--latest` therefore reports runtime `claude` from the Claude store, not a Codex rollout. The fallback notice states the observable consequence rather than the intent, and `--runtime codex` remains the honest way to probe a Codex session. Closing the gap means routing an `env-override` host into probe selection, which changes the `cli-agent-context` contract and belongs to a later capability change.
