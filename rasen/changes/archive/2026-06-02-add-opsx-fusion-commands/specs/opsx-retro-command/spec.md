...
## Purpose
Defines the `/opsx:retro` engineering retrospective command that analyzes what shipped and produces actionable improvement insights.
...
The command SHALL support 3 scopes: change-scoped, general, and global.
...
- **THEN** the retro SHALL analyze recent git commits and repository activity
- **AND** SHALL produce insights based on commit patterns, frequency, and code areas touched

#### Scenario: Global retro invocation

- **WHEN** agent executes `/opsx:retro global`
- **THEN** the retro SHALL delegate to gstack /retro global for cross-project analysis
- **AND** SHALL produce insights spanning multiple repositories if available

### Requirement: Change-Scoped Artifact Reading

Change-scoped retro SHALL read all available change artifacts from the change directory.

#### Scenario: Full artifact set available

- **WHEN** running a change-scoped retro
- **AND** the change directory contains proposal.md, design.md, tasks.md, review-report.md, qa-report.md, and ship-log.md
...
- **WHEN** running a change-scoped retro
- **AND** some artifacts are missing from the change directory
...
- **THEN** the report SHALL be written to `openspec/changes/<name>/retro.md`
...
- **THEN** the report SHALL be written to `openspec/retro-latest.md`
