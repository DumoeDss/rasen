...
## Purpose
Defines the enhanced verification command that combines OpenSpec artifact consistency checks with gstack expert reviews (code review, security, QA, design review).
...
Reports SHALL be saved to the change directory.

#### Scenario: Report files written after verification

- **WHEN** verification completes
- **THEN** the review report SHALL be saved to `openspec/changes/<name>/review-report.md`
- **AND** the CSO report SHALL be saved to `openspec/changes/<name>/cso-report.md` (if /cso was invoked)
- **AND** the QA report SHALL be saved to `openspec/changes/<name>/qa-report.md` (if /qa was invoked)
