...
While session supervision lives in the foreground `rasen ui` server (before daemon residency), a cleanly-exiting server SHALL terminate all live supervised sessions via the tree-kill path with termination reason `server-shutdown` before exiting, because an in-memory registry leaves an orphaned agent run unobservable and unkillable. This posture SHALL be documented as the pre-residency behavior: a force-killed server can still orphan sessions, and daemon residency is the designated resolution.

#### Scenario: Clean shutdown leaves no orphaned session processes
- **WHEN** the server shuts down cleanly while sessions are running
- **THEN** each live session's process tree is terminated before the server exits, with termination reason `server-shutdown`
