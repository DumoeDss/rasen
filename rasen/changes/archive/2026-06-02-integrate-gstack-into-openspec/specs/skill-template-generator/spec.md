...
## Purpose
Provide a build-time generator (`scripts/gen-skill-docs.ts`, migrated from gstack) that produces SKILL.md files from SKILL.md.tmpl template sources in the `skills/` directory. The generated SKILL.md content is then consumed by TypeScript template functions in `src/core/templates/experts/`.
