...
- [x] 8.6 Write parity test: CLI, Management API, and Operations consume same fixture and see same parallel/choice sections
  - Projector, CLI (`pipeline status --json`) and Management API (`handleRunDetail`) planes: `test/core/change-run/projector-parallel-choice.test.ts` — all three are compared on ONE fixture Record and must produce byte-identical `parallel/1` and `choice/1` sections.
  - Operations/UI plane: `packages/ui/test/components/ecp4-parallel-choice-parity.test.tsx` — per the existing `cross-plane-parity.test.tsx` convention the UI plane is asserted under `packages/ui/test/`, by rendering the REAL `OperationsSection` component against the SAME fixture's projected sections (their provenance is documented in that file's header). The consumer it exercises is real production code: `getParallelSection`/`getChoiceSection` in `packages/ui/src/api/types.ts` (mirroring the core `ParallelSectionView`/`ChoiceSectionView` wire types) plus the `ParallelSection`/`ChoiceSection` renderers in `packages/ui/src/components/OperationsSection.tsx`, which show member statuses, join state, budget usage, the projected member frontier, the selected choice outcome and each branch's active/inactive state. Three of the UI assertions feed deliberately incoherent sections (counts, join state and branch activation that disagree with the members/outcome) so a UI that recomputed those values client-side instead of consuming them would fail.
