...
- **Code**: `src/commands/ui.ts`, `src/commands/config.ts` (the `config ui` action only), a new shared launch module under `src/commands/`, `src/core/management-api/{router,server,changes,runs}.ts` (route-group assembly, path normalization, cached home resolution), `src/core/completions/command-registry.ts` (a top-level `ui` entry — the shell-completion registry requires parity with every visible Commander command). `src/cli/index.ts` needed no change: `rasen ui` was already registered there in batch 1, un-hiding it only required dropping `{ hidden: true }` inside `ui.ts` itself. UI package: `packages/ui/src/app.tsx` (routes), `packages/ui/src/components/Layout.tsx` (navigation), `RelaunchNotice.tsx` (wording), `index.html` (title).
