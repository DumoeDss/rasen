...
- [x] 6.6 Confirm `grep -rl "TBD - created by archiving" rasen/specs/` is empty after rehearsal and real archive, the existing portable-project-knowledge Purpose remains verbatim, the shared dirty worktree and sibling change directories are untouched and unstaged, and `package.json` version is unchanged.
