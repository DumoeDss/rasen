...
Turn a Change's ending from an implicit side effect of moving its directory into one explicitly declared, provable terminal state. A Store v2 Change ends as `landed`, `superseded`, `cancelled`, or `abandoned`; only `landed` may change a canonical spec, and only after the code it describes is proven reachable from the code ref its own target line declares. A Change with no code says so in its committed metadata rather than at the moment it would otherwise have to prove something. A superseding Change is resolved to real, verified scope evidence rather than to a name, and no Change is ever filed into a target line other than the one frozen in its identity. Every uncertain fact — an unresolvable ref, an indeterminate Git answer, an unverifiable successor, a missing workspace pair — refuses instead of being defaulted into a record that would then validate.
