...
Give a legacy flat Store exactly one explicit, previewable, resumable route into layout v2: inventory its planning content per Git ref, decide project ownership only from auditable evidence, refuse rather than guess when evidence is absent, contradictory, or shared, publish the project partitions atomically with a recoverable staging protocol, and leave a committed receipt that explains every decision afterwards.
