...
> Stacked delta: the REMOVED requirement below is the ADDED text of the pending `ui-config-redesign-wildcard-config` (W3-enabler) change's delta to this spec — that change must archive before this one.

## REMOVED Requirements

### Requirement: Wildcard configuration key families

**Reason**: The family set is enumerated as exactly four; re-pointing `rasen pipeline agents` at configuration requires a fifth family carrying per-role runtimes. Replaced by "Wildcard configuration key families including the pipeline runtime namespace".
**Migration**: The pattern mechanism, structural placeholder validation, the four existing families, and every behavior carry over verbatim; only the runtime family is added to the enumeration.
