...
The face therefore derives server-side **only when the granted Action is the bound Teacher**. Every other consultation-driven Action keeps its caller-transported bytes, which its own turn-input binding already authenticates. Trusting `envelope.turnInput` uniformly was rejected: that would hand the Teacher's question back to the caller and undo Decision 13.
