...
- [ ] 12.8 Close `RC-005`: remove terminal clients from the map on every success/failure/timeout path and prove no lifecycle leak. **In progress:** one shared retention lifecycle rule (`scope-retention.ts`) across all three maps; POSIX + win32 maps landed and mutation-discriminated; the legacy `clients` map is in the byte-pinned `native-process-scope.ts` and its edit + rebaseline (`a070733c -> 3e74b2c2`) HOLDS pending explicit LEAD authorization. See `evidence/closure-integration-disposition.md`.
