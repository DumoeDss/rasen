...
- [ ] 8.5 Full suite green: `pnpm lint`, `pnpm build`, `pnpm test`. — `pnpm lint` and `pnpm build` are clean. The LEAD reserved the full `pnpm test` run for itself before ship (two concurrent vitest batches produced spurious timeouts for another worker), so this stays open by instruction; a broadened targeted sweep of every affected area is green.
