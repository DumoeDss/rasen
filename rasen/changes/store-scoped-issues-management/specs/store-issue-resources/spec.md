...
Give cross-project work the only representation layout v2 permits. Every ordinary Change has exactly one project owner and none may have zero, so work spanning two projects lives as a Store-level Issue whose Execution Plan revisions *reference* per-project Change instances by their stable portable identity. An Issue owns no Change, contains no planning content, and writes nothing into what it references; a plan revision is immutable, ordinal-addressed, and validated against real Store evidence before it is published; and every node names its project and its target line whether or not its Change exists yet, so single ownership is explicit from the first draft rather than reconciled at the end.
