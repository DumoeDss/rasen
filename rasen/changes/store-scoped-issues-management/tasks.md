...
- [ ] 11.11 Before the archive step, run the pairwise requirement-title and scenario-set comparison against `rasen/specs/` plus children 3, 4, and 5's deltas, and confirm the `management-http-api` and `board-ui` MODIFIED blocks still byte-match their post-sibling canonical titles and repeat every current scenario.
