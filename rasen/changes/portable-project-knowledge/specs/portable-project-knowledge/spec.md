...
### Requirement: A project's knowledge travels between machines only when the user exports a bundle

Rasen SHALL provide a way to export a project's own learned knowledge as a single portable file, and to import such a file into the same project on another machine. Nothing SHALL move a project's knowledge between machines on its own: no ordinary command, no setup or update, and no diagnosis SHALL produce, transmit, or read a bundle. Export SHALL name the project by the project's identity, SHALL read the project's canonical knowledge without modifying it, and SHALL report the same facts in human and machine-readable output.

#### Scenario: Export produces one file and changes nothing else

- **WHEN** a project's knowledge is exported to a destination
- **THEN** exactly one bundle file is produced at that destination
- **AND** the project's stored knowledge, the checkout, and the machine's registrations are unchanged

#### Scenario: Nothing moves knowledge on its own

- **WHEN** ordinary commands, setup, update, and diagnosis run for a project that has knowledge and a declared Store
- **THEN** no bundle is written, read, or transmitted by any of them

#### Scenario: The bundle names the project by identity, not by checkout

- **WHEN** the same project is exported from two different checkouts on one machine
- **THEN** both bundles name the same project identity
- **AND** neither names the checkout it was exported from

#### Scenario: Human and JSON report the same facts

- **WHEN** the same export runs once in human mode and once with machine-readable output
- **THEN** both report the same project, the same record count, the same destination, and the same warnings

#### Scenario: An existing destination file is never replaced

- **WHEN** the named destination already holds a file
- **THEN** the export refuses, names the existing file, and writes nothing
