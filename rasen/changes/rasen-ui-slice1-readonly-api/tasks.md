...
- [x] 1.2 Create `changes.ts` handler: enumerate via `getActiveChangeIds`, load per-change status via `loadChangeContext` + artifact-graph completion, task counts via `countTasksFromContent`; error envelope (not empty success) when no project resolves (spec: changes listing matches the workflow's active-change definition)
