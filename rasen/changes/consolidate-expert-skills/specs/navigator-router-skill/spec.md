...
**Reason**: The standalone navigator duplicates `rasen-help` and adds an extra user/model routing identity.

**Migration**: Invoke `rasen-help`; it conditionally reads the bundled navigator map for broad routing questions.

#### Scenario: Standalone navigator is retired

- **WHEN** a fresh install or update completes
- **THEN** no `rasen-navigator` skill SHALL be generated
- **AND** `rasen-help` SHALL contain the navigator reference

### Requirement: Navigator registered as an expert with count +1

**Reason**: Navigator is now a host-owned reference rather than a catalog expert.

**Migration**: Profile and catalog consumers select `help`; expert counts and localized metadata cover only surviving experts.

#### Scenario: Navigator registration and picker row are absent

- **WHEN** the catalog, profile picker, and expert documentation are inspected
- **THEN** no `navigator` expert entry or `/navigator` row SHALL appear
- **AND** help SHALL remain available through the normal workflow registry
