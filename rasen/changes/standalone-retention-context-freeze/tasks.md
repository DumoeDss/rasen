...
- [x] 7.10 Make the read-failure test portable: its fixture (a run-state path underneath a regular file) reports `ENOTDIR` on POSIX but `ENOENT` on Windows — the one code the seam must treat as a genuine absence — so the assertion held on macOS/Linux and failed on the Windows CI shard. The errno is now injected through the same partial-`node:fs` mock the crash-safety tests use, which tests the discrimination itself on every platform.
