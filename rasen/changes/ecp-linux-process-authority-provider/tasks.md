...
- [x] 7.8 Run and record natural empty, exact code exit, exact signal exit, root-exit-with-live-descendant, recursive force, prepared abort, and published abort actual-kernel oracles. [NARROWS: the published-abort row moves to the upgrade path; the other seven oracles stay]
...
- [ ] 11.9 Run local ship only after every implementation, actual-kernel, verification, and review task above is complete; create a path-scoped child commit with no push, child PR, production-default switch, or unrelated retained file. [NARROWS: "every ... task above" now excludes tasks marked moved; Section 9 and broker rows no longer gate local ship]
