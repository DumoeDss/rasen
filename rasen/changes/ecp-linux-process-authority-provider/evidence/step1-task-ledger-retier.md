...
| 7.8 | [x] | NARROWS | (a) | The task text names seven oracles. Six stay (natural empty, exact code exit, exact signal exit, root-exit-with-live-descendant, recursive force, prepared abort); the "published abort" row accompanies the publication machinery (WSL-R4-M04). An earlier revision of this row read "Seven of the eight named oracles stay", which contradicted both the task text and this row's own six-item list; corrected 2026-08-07, no verdict change. |
