...
## Planner findings（child 4 task-detail propose,2026-07-21）

21. **roster-assembly 定形:新增只读端点 `GET /api/v1/tasks/:id?space=`,非纯客户端拼装**。四件事既有端点都取不到,故必须服务端:(a) Task 的**归档 children**(archive 端点尚不存在=child 5)、(b) 父容器的 `portfolio-run.json` deps(父无 proposal.md→不在 `/changes`/`/runs`)、(c) 容器**全归档时**的 Task 存在性/kind(此时 `/changes` 空)、(d) 单件 Task 的 task **清单条目**(既有 helper 只给计数)。端点复用 `getActiveChangeIds`/`getArchivedChangeIds`/`readPortfolioState`/`getTaskProgressForChange`/`buildChangeRunEntry`,**零 mint 零写**,空间解析与 `/changes` 同(显式 selector→registry,缺省→launch 兜底,无 root→400)。给 child 5:archive 页应自建**全局** archive-listing 端点;本 child 的 `/api/v1/tasks/:id` 只报**单个 Task 的**归档 children,不冲突;可复用我导出的 `findPortfolioContainers`/`portfolioOf`(child 3 私有→本 change 改为 export)。
22. **磁盘现状(实现者/child 5 必读)**:`ui-space-redesign` **没有 portfolio-run.json**、**零归档 child**(5 个 slice 全活跃)。故 deps 段今天必然空、archive 段今天必然空——两条路径**必须建但必须优雅退化为空**(deps 缺→"无声明依赖"不报错;archive 缺→只列活跃)。别把"当前无归档"误当"不用处理归档"。
23. **归档 children 名带日期前缀**:`getArchivedChangeIds(root)` 返回 `YYYY-MM-DD-<name>`(in-repo + machine-home archive 的 sticky-union,去重不带位置)。归属匹配须**先剥日期前缀**再 `portfolioOf`;读归档 child 的 task 进度须**先探 in-repo archive 目录、再探 `home.archiveDir`**(getArchivedChangeIds 不告诉你 which dir)。child 5 建 archive 页会碰同一位置解析问题,可把此逻辑抽成共享 helper。
24. **wire mirror 新增两型**:`TaskChildDetail`/`TaskDetailResponse` 进 server `wire-types.ts` + UI `api/types.ts` 双镜像 + `satisfies` fixture(承既有纪律,禁 `as`);复用 `ChangeSummary`/`ChangeRunEntry`/`StageStatus`。archived child `summary:null`+`archived` 旗标(不伪造 artifact 事实),`taskProgress` 提到 child 级(archived 无 summary 也带计数),UI 列 = `archived?'done':deriveColumn(summary,run).column`。
