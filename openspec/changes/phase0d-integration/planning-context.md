...
## planner 调研纪要（0d-sidecar-install）

> APPEND 于 2026-07-06（HEAD=3a70bd4，sibling add-context-handoff 已归档、navigator 已 ship；0d-sidecar-install propose 完成）。纯 TS，无 sibling 冲突（init.ts 已稳定）。

### 缺陷与落点
- `init.ts`(~552) 与 `update.ts`(~200) 共享同一 install loop：`for {template,dirName} of skillTemplates` → generateSkillContent → writeFile(SKILL.md)，**只写 SKILL.md**。registry 条目已带 `workflowId`（=源目录名 `skills/gstack/<workflowId>/`）。
- sidecar 盘点：review 4 个 .md、qa references/templates、0c 八个 .md、investigate/scripts/hitl-loop.template.sh、careful/freeze bin/*.sh；**异类 browse**=vendored bun 包（src 205K + test 252K 的 .ts）必须排除。

### 方案拍板：**扩展名 allowlist（.md/.sh）+ 跳过 browse**（写进 design D1）
- 三方案比较：(a) 整目录减 exclude-list（脆，须枚举 browse 各重目录并维护）；(b) 扩展名 allowlist + 保留子目录（最轻、自维护、构造性排除 browse .ts 树）；(c) SkillTemplate sidecar manifest（最显式但每 skill/每新 sidecar 都要声明，authoring 负担）。**选 (b)**。
- 规则：递归 copy `.md`(除 SKILL.md)+`.sh`，保留子路径，排除 `*.tmpl`；`workflowId==='browse'` 整个跳过（browse 二进制经 {{BROWSE_SETUP}} 单独装，非 sidecar）。附带把 careful/freeze 的 bin/*.sh 也装上（bonus；但其 hook 路径用 `../<skill>/bin/` 无前缀、装机目录是 `openspec-gstack-<name>`，属先存 dir-name 路径错配，本次不修只保证文件到位）。
- 集中到 `skill-generation.ts::copySkillSidecars(workflowId,targetDir)`（源目录用与 expert 模板同样的 import.meta.url 相对解析），init+update 都调；源目录缺失时优雅 no-op（对齐 expert 模板 readFileSync try/catch）。

### ⚠️ 深层 flag：`skills/` 不进 npm 包（先存、非本次修）
- `build.js` 只跑 gen-skill-docs+tsc；package.json `files`=dist/bin/schemas/pipelines，**无 skills/**。故源树只在 **checkout** 存在——正是本 fork 的用法（`node bin/openspec.js …` 从 repo 跑）。**现有 expert 模板 readFileSync 已依赖此**（published 包里会命中 catch 报 "Skill file not found"）。本 change 与现状一致（checkout 能用、否则优雅跳过），不 regress。修 published 分发是独立决策——naive 加 skills/ 到 files 会把 browse 450K .ts 树发到 npm；正解是选择性发 `skills/**/*.{md,sh}`(+生成的 SKILL.md)。已 flag，非本次。

### 幂等/卸载
- init/update 覆盖写（幂等）；stale sidecar（源删了的）v1 不 prune（可接受、已注，全镜像+prune 有误删用户编辑风险）；removeSkillDirs 删整目录→sidecar 随之删（干净卸载）。升级既有安装 = 下次 init/update 多出文件（预期、已写进 proposal 行为变化）。

### 0d-sidecar-install 产物（已 validate --strict 通过）
- proposal（含深层 shipping-gap flag + 行为变化说明）/ design（D1 三方案对比选 b、D2 集中 helper、D3 优雅缺失、D4 幂等/卸载、D5 skills 不发包 flag）/ specs（1 ADDED：`skill-sidecar-install`）/ tasks（4 组：helper / init+update 接线 / 测试含实跑 / verify）。4/4 complete。
- **实跑验证**（tasks §3.2/§4.5）：临时目录 openspec init+update 抽查 investigate/scripts 与 review/checklist.md 落地、browse/src 不落地、update 幂等。

### phase0d-integration 全 portfolio 规划完结
- 5 子：absorb（归档）/ router（归档）/ sidecar-install（待 apply）——**注**：LEAD 播种的三子边界（absorb/router/sidecar-install）已全部 propose 完毕。
