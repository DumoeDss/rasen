# Delta Spec: orchestration-handoff
...
The `/opsx:auto` entry SHALL probe the LEAD's own transcript (`openspec agent context --latest`) once before starting the pipeline and remind the user — without blocking — when usage meets the session threshold, suggesting `/opsx:handoff`.
...
- **THEN** the LEAD SHALL surface a one-line reminder offering handoff and continue only on the user's say-so at that moment; below threshold it proceeds silently
