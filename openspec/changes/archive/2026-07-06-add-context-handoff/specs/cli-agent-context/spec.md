...
- **WHEN** a user runs `openspec agent context --transcript <path> --json` against a Claude Code transcript jsonl
...
- **WHEN** a user runs `openspec agent context --latest`
