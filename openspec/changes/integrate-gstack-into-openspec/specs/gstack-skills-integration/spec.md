...
## Purpose
Migrate gstack expert skills into OpenSpec by integrating them through OpenSpec's existing SkillTemplate system (`src/core/templates/` + `skill-generation.ts` registry). Skill source files (.tmpl) are stored in `skills/`, TypeScript template functions in `src/core/templates/experts/`, and the registry in `skill-generation.ts` drives installation via `openspec init`.
