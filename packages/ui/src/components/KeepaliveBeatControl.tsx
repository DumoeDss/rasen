 ...
 * control for the global-only `keepalive.beatSeconds` key: two built-in presets
 * — 100s (fast, compatible with the shell tool's default timeout) and 270s
 * (economy, the registry default) — plus a bounded 90–280 custom input. Writes
...
/** Built-in presets: the value each button writes. */
const FAST_PRESET = 100;
...
function validateBeat(value: number): string | null {
  if (!Number.isInteger(value) || value < BEAT_MIN || value > BEAT_MAX) {
    return `Must be an integer between ${BEAT_MIN} and ${BEAT_MAX}`;
  }
  return null;
}
...
export function KeepaliveBeatControl({
  ...
  function commit(value: number) {
    const invalid = validateBeat(value);
    if (invalid) {
      setError(invalid);
      return;
    }
  ...
  const active = effective === FAST_PRESET ? 'fast' : effective === ECONOMY_PRESET ? 'economy' : 'custom';

  return (
    <div class="keepalive-beat" data-testid="keepalive-beat-control" data-key={key} title={entry.definition.description}>
      <span class="keepalive-beat__label">Keepalive beat</span>
      <div class="keepalive-beat__presets" role="group" aria-label="Beat presets">
        <button
          type="button"
          class={`member-chip${active === 'fast' ? ' member-chip--selected' : ''}`}
          data-testid="keepalive-preset-fast"
          aria-pressed={active === 'fast'}
          disabled={pending}
          onClick={() => commit(FAST_PRESET)}
        >
          Fast ({FAST_PRESET}s)
        </button>
        <button
        ...
        >
          Economy ({ECONOMY_PRESET}s, default)
        </button>
      </div>
      <label class="keepalive-beat__custom">
        ...
        <button
          ...
          onClick={() => {
            if (custom.trim() === '') {
              setError(`Must be an integer between ${BEAT_MIN} and ${BEAT_MAX}`);
              return;
            }
      ...
      <span class="keepalive-beat__hint" data-testid="keepalive-timeout-hint">
        Suggested shell tool timeout ≈ {hintBeat + TIMEOUT_MARGIN_SECONDS}s (beat + {TIMEOUT_MARGIN_SECONDS}s)
      </span>
      <span
