...
- `openspec pipeline resume <change> --json` reads the run-state and exposes `workers`, `inProgressStages`, `openFindings`, and the portfolio's `planner`.
...
The output of `openspec pipeline resume --json` can still be called `workers`, but each entry carries a `runtime`, for example:
...
1. LEAD writes `openspec/changes/<id>/planning-context.md`.
...
4. LEAD runs `openspec validate <change> --strict` or at least `openspec status --change <id> --json` for structural validation.
5. The Codex thread is written to `stages.propose.worker.threadId` in `auto-run.json`.

Resuming the same propose later:

1. `openspec pipeline resume <change> --json` returns the propose worker's `threadId`.
...
- After the fix, LEAD computes the fix delta and writes it to `openspec/changes/<id>/review-delta-round-<n>.md`.
...
- current artifacts: a summary from `openspec show <change>` / `openspec status --json`.
...
- propose: run `openspec validate <change>` or `openspec status --json`.
...
- Have `openspec pipeline resume --json` emit runtime-aware workers.
...
- Add `openspec codex setup/status` or an internal availability check.
