...
**`/rasen-explore` is your thinking partner. Reach for it whenever you have a problem but not yet a plan.** It investigates your codebase, weighs options with you, and clarifies what you actually want, all before a single artifact or line of code is created. When the picture is clear, it hands off to `/rasen-propose`.
...
- Transition to `/rasen-propose` when you're ready.
...
You can say it in plain language ("let's turn this into a change") or run `/rasen-propose <name>` directly. Either way, the exploration you just did becomes the foundation of the proposal, not throwaway chat.

If you use the expanded command set, explore can hand off to `/rasen-new-change` instead, for step-by-step artifact creation. See [Workflows](workflows.md).
...
- **Explore again mid-change.** Stuck during `/rasen-apply-change`? You can step back and explore a sub-problem, then return.
...
**What it costs:** a little patience. Explore is a conversation, so it's slower than firing off `/rasen-propose` and hoping. For work you genuinely understand already, that extra step is pure overhead, and you should skip it.
