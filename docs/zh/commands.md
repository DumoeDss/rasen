...
以迭代循环把变更推进到「真正干净」：`审查 → 分诊 → 修复 → 复审（增量）→ {通过 | 继续循环 | 上报}`。它不重新实现审查器 —— 每一轮都委托给始终安装的 `openspec-gstack-review` 引擎。本命令只负责循环、按修复规模分诊、作者≠验证者不变式、终止与上报。属于可选项（不在 `core` 配置中）。
...
**它做什么：**
- 通过 `openspec-gstack-review` 跑一轮审查，然后按修复规模对每条发现分诊
