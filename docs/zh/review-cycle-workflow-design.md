...
| 步骤 | 做什么 | 引擎 |
| --- | --- | --- |
| **review** | 对照该 change 的 specs/tasks 评审已实现的 diff；按严重程度（Blocker/Major/Minor）产出问题清单，每条问题都关联到一处 file:line，并（在可能时）关联到一条规范的 `#### Scenario`。 | 复用 `openspec-review`（`src/core/templates/experts/review.ts`）。 |
