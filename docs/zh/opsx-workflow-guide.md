...
| 归档 | `/rasen:archive` / `/rasen:bulk-archive` | 归档 change，把 delta spec 合并进 canonical specs（流水线中固定用 `model: sonnet` 跑）| 归档目录 + 更新的 specs |
| 合并 spec | `/rasen:sync` | 把 delta specs 合并进主 specs | 更新的 specs |
...
不带结构的探索对话：查代码、比选项、画图。想法成型后可转入 `/rasen:propose`（默认）或 `/rasen:new`（expanded）。

### 3.2 `/rasen:office-hours` — 先验证需求该不该做
...
建 `rasen/changes/<id>/` 并生成实现前所需的全部产物（spec-driven：proposal → specs → design → tasks），停在「可 apply」状态。要分步控制就用 expanded 的 `/rasen:new` + `/rasen:continue`。
...
### 3.6 `/rasen:verify` / `/rasen:verify-enhanced` — 验证
...
| 归档并合并 spec | `/rasen:archive`（或 CLI `rasen archive`）|
| 复盘 | `/rasen:retro` |
