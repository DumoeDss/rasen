...
```nix
{
  inputs = {
    ...
    openspec.url = "github:DumoeDss/rasen";
  };
...
```bash
npm install -g rasen@latest   # 或 pnpm/yarn/bun 的等价命令
...
没有 `rasen uninstall` 命令，因为 OpenSpec 不过是一个全局包，外加你项目里的若干文件。移除它只需几个手动步骤，并且这些步骤都不会触碰你的源代码。

**1. 移除全局包：**

```bash
npm uninstall -g rasen   # 或：pnpm rm -g / yarn global remove / bun rm -g
```
