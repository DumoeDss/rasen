...
此后，该仓库中的 `openspec instructions` 输出（包括逐产物和 `apply` 两种界面、JSON 与人类模式）会携带每个被引用 store 的规格说明索引——spec id、取自各 spec Purpose 部分的一句话摘要，以及获取命令（`openspec show <spec-id> --type spec --store <id>`）。该索引在每次运行时从已注册的检出实时构建；spec 内容从不被复制到输出中。
