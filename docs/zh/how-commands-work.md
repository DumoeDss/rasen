...
- `/rasen:...` 命令在你的 **AI 助手的聊天**里运行。（例如：`/rasen:propose`。）
...
**斜杠命令（聊天那一半）。** 像 `/rasen:propose` 和 `/rasen:apply` 这样简短的命令，你把它们打进 AI 助手。这些命令告诉 AI 去遵循 OpenSpec 工作流：起草 proposal、编写 specs、按任务清单构建、完成后归档。你在 Claude Code、Cursor、Windsurf、Copilot，或你使用的任何助手里输入这些命令。

```text
/rasen:propose add-dark-mode    （打在你的 AI 聊天里）
/rasen:apply                    （打在你的 AI 聊天里）
/rasen:archive                  （打在你的 AI 聊天里）
```
...
3. 看自动补全：如果 OpenSpec 已经安装，你一边敲斜杠，就会看到 `/rasen:propose`、`/rasen:apply` 等陆续出现。
...
- `/rasen:apply`：按变更的任务清单逐项构建
- `/rasen:sync`：把变更的 spec 更新合并进你的主 specs（通常是自动的）
- `/rasen:archive`：完成一项变更并把它归档
