...
所有 rasen 术语汇集于一处，用通俗的语言加以定义。先浏览一遍，之后阅读其余文档会顺畅许多。
...
**增量规格（Delta spec）。** 变更内部的一份规格，只描述正在改变的内容，使用 `ADDED`、`MODIFIED` 和 `REMOVED` 区段，而不是把整份规格重述一遍。这正是让 rasen 能干净地修改既有系统的关键。参见[概念](concepts.md#增量规格delta-specs)。
...
**任务（`tasks.md`，Tasks）。** 实现清单，带复选框。AI 会在 `/rasen-apply-change` 期间逐项推进，并随着进展勾选条目。
...
**同步（Sync）。** 把一个变更的增量规格合并进主规格，但*不*归档该变更。通常是自动进行的（归档时会主动提议同步），不过对于长期推进的变更，也可以单独通过 `/rasen-sync-specs` 来执行。参见[命令](commands.md#rasensync)。

## 工作流与命令

**制品工作流（the artifact workflow）。** 当前 rasen 的标准工作流，围绕灵活的动作而非僵化的阶段构建。它的斜杠命令都以 `/rasen-` 开头。曾用名 "OPSX"，已退役的代号。参见 [制品工作流](artifact-workflow.md)。
...
**CLI。** 你在终端里运行的 `rasen` 程序。它负责初始化项目、列出并校验变更、打开仪表盘、执行归档。它是 rasen 的"终端那一半"。参见 [CLI](cli.md)。

**技能（Skill）。** 一个指令文件夹（`.../skills/rasen-*/SKILL.md`），你的 AI 助手会自动探测并遵循它。技能是把 rasen 工作流交付给助手的、正在兴起的跨工具标准。

**命令文件（Command file）。** 针对某个工具的斜杠命令文件（`.../commands/rasen-*`）。当 delivery 为 `both` 时，与技能一并安装的可选补充。你很少需要直接改动这些文件。
...
**项目配置（`rasen/config.yaml`，Project config）。** 针对项目的设置：默认模式、注入到每次规划请求中的 `context:`，以及针对单个产物的 `rules:`。这是把你的技术栈和约定告诉 rasen 最简单的方式。参见[自定义配置](customization.md#项目配置)。

**上下文注入（Context injection）。** 把项目背景放进 `config.yaml` 的 `context:` 字段，从而让它被自动加到 AI 生成的每一份产物里。这比寄望 AI 去读另一个单独文件要可靠得多。

**依赖图（Dependency graph）。** 由产物的 `requires:` 关系形成的有向图。它是一个 DAG（有向无环图：箭头只向前指、绝不形成环），rasen 借助它来判断你接下来可以创建什么。
...
**Store。** 一个独立的仓库，其全部职责就是规划。它拥有你早已熟悉的同样的 `rasen/` 结构（specs 和 changes），外加一个小型的身份文件。你在自己的机器上按名字注册它一次，之后任何 rasen 命令都可以从任何地方在其中运作。
...
**工作上下文（Working context）。** `rasen context` 为当前仓库组装出来的内容：它的 Rasen 根，加上它引用的每一个 store，以及各自如何获取。这是"我正在和什么打交道？"的答案。
