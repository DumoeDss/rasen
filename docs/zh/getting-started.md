...
>
> - `openspec ...` 命令（如 `openspec init`）在你的**终端**里运行。
> - `/opsx:...` 命令（如 `/opsx:propose`）在你的 **AI 助手的聊天里**运行——就是你让它写代码的那个对话框。
>
...
> **还不确定要构建什么？从 `/opsx:explore` 开始。** 它是一个零成本的思考伙伴——读你的代码库、权衡各种选项，在任何产物或代码存在之前，把一个模糊的想法打磨成具体的计划。当图景清晰了，它会把工作交接给 `/opsx:propose`。这是与一个“否则会自信满满地造错东西的 AI”协作时，最值得养成的单一习惯。参见 [探索指南](explore.md)。

## 工作原理

OpenSpec 帮助你和你的 AI 编程助手在写下任何代码之前，就先就“要构建什么”达成一致。
...
当你在斟酌要做什么时，从 `/opsx:explore` 开始；已经胸有成竹时，则直接跳到 `/opsx:propose`。explore 包含在默认 profile 里，所以你想用的时候它总在。
...
默认的全局 profile 是 `core`，包含 `propose`、`explore`、`apply`、`sync` 和 `archive`。你可以通过 `openspec config profile` 再加上 `openspec update` 来启用扩展工作流命令。

## OpenSpec 创建的内容

运行 `openspec init` 后，你的项目会拥有如下结构：
...
如果你启用了扩展工作流 profile，也可以分两步来做：先 `/opsx:new`，然后 `/opsx:ff`（或用 `/opsx:continue` 逐步推进）。
