...
> 配套阅读：`docs/artifact-workflow-guide.md`（命令总览）、`docs/review-cycle-workflow-design.md`（评审循环设计）、`skills/experts/docs/`（专家技能架构）。
> 术语说明：写这份快照时，融合工作流层被称为「OPSX」。这个名字后来已退役；现在的文档里同一个层称为**制品工作流（the artifact workflow）**（见 `docs/artifact-workflow.md`）。下文出现的「OPSX」描述的是当时的叫法，不是现在的名字。
...
| **OPSX**（已退役的名字；现称「制品工作流」） | OpenSpec 之上的「融合工作流层」：`/opsx:auto` 编排器、流水线注册表、ship/verify-enhanced/office-hours/retro 等命令、LEAD+worker 编排模型 | 吸收 grill/gstack 后长出的工作流层 |
