...
> 本文是「现状快照 + 来龙去脉」，不是变更日志。变更日志看 `rasen/changes/archive/` 各 change 的 retro。
...
| **OPSX** | OpenSpec 之上的「融合工作流层」：`/rasen:auto` 编排器、流水线注册表、ship/verify-enhanced/office-hours/retro 等命令、LEAD+worker 编排模型 | 吸收 grill/gstack 后长出的工作流层 |
...
1. **`gstack-skills-integration`** —— 把 gstack 专家技能初次接入 OpenSpec（模板源 + sidecar + 注册 + `rasen init` 安装）。
...
8. **`remove-parallel-lifecycle-skills`** —— 删除 10 个平行生命周期专家（`/autoplan`、`/plan-*-review`、`/canary`、`/document-release`、`/setup-deploy` 等），并把 `ship`/`retro` 契约吸收进 `/rasen:ship`、`/rasen:retro` 自包含工作流。专家名册 30→20。
...
│  → ship → archive → retro    驱动器：/rasen:auto             │
├─────────────────────────────────────────────────────────────┤
│  底层：rasen CLI（确定性的状态读写/校验/归档基座）         │
...
| `/ship` + `/land-and-deploy` | → `/rasen:ship`（land-and-deploy 变为 `--deploy`） |
| `/retro` | → `/rasen:retro` |
...
`/rasen:auto` 是驱动器：LEAD（编排者，不亲自动手写产物）按流水线 DAG 把每个 stage 派给一个**角色隔离的叶 worker**（planner/implementer/reviewer/fixer/shipper），worker 调用该 stage 对应的 OPSX 技能。关键不变量：

- **author ≠ verifier**：评审者不能是作者；修复必须由非作者复核。
- **change 目录是黑板**：stage 间通过 `rasen/changes/<name>/` 的产物交接（proposal/design/tasks/specs/review-report/ship-log），不靠共享���存。
...
`src/core/templates/experts/<name>.ts` 是专家技能的**唯一权威源**——每个 getter 返回一个 `SkillTemplate`，指令体是 TS 模板字符串。共享块（PREAMBLE、BROWSE_SETUP、SPEC_REVIEW_LOOP 等 14 个）抽到 `src/core/templates/experts/_shared.ts` 常量。`rasen init`/`update` 从这些模板生成安装侧的 `SKILL.md` + sidecar。
...
| 工作流命令 | `/ship`、`/retro` | `/rasen:ship`、`/rasen:retro` |
...
- **`browse`** 原本是一个 gstack vendored 的无头浏览器工具（真实 Chromium，作为黑盒并入 `skills/experts/browse/`，自带 `src/`、`test/`、`scripts/build-node-server.sh`）。fork 已**移除**它，改用 **`chrome-use`**——后者通过 `skills/experts/chrome-use/` 下的 vendored Node 代理经 CDP 操控用户自己的 Chrome，而不再分发编译好的浏览器二进制。
...
- **navigator 的 `/rasen:ship` 简介未提三模式**：`navigator.ts:22` 仍写「test, push, open the PR」，没反映 §4.3 的三交付模式 + 证据门。一句话级修复（`ship-delivery-modes` 评审遗留 F3）。
