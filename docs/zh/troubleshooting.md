...
- 在提出变更时给出更详细的描述。
- 使用扩展的 `/rasen:continue`，一次创建一个产物并逐一评审，而不是用 `/rasen:ff` 一次性全部生成。
...
1. **文件名不对。** 必须是 `rasen/config.yaml`，而不是 `.yml`。
...
重启你的 IDE。技能在启动时被检测。如果仍然不出现，运行 `rasen update` 并检查[支持的工具](supported-tools.md)里的文件位置。
