...
A profile decides which slash commands get installed. **Full** (the default) installs every workflow. **Core** slims down to `propose`, `explore`, `apply`, `sync`, `archive`, and **custom** lets you pick any subset. Switch with `openspec config profile`, then apply with `openspec update`.
