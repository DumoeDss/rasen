...
If you ever type `/rasen:propose` into your terminal and nothing happens, this page is why. You are talking to the wrong half of OpenSpec. Slash commands are not terminal commands. They are instructions you give to your AI coding assistant, in the same chat box where you'd normally type "add a login form."
...
3. Watch the autocomplete: if OpenSpec is installed, you'll see `/rasen:propose`, `/rasen:apply`, and friends appear as you type the slash.
...
When you run `rasen init` (or `rasen update`), OpenSpec writes small files into your project so your AI tool can find the workflow. Depending on your tool and settings, these are **skills**, **commands**, or both.
...
There's also an **expanded** set for people who want finer control (`/rasen:new`, `/rasen:continue`, `/rasen:ff`, `/rasen:verify`, `/rasen:bulk-archive`, `/rasen:onboard`). You turn it on with `rasen config profile`, then apply it with `rasen update`.
