...
An agent can't perceive its own context usage — it can only **measure** it. `rasen agent context` reads the precise usage from the API usage recorded in the transcript (`--latest` measures the main session itself, `--transcript <path>` measures a specific worker, `--json` outputs `{ model, contextTokens, limit, pct, remainingTokens }`). The two flags fail differently, on purpose: a bare `--latest` on a host whose harness has no context-probe adapter answers `{ available: false, reason: "unsupported-host" }` at exit 0 — that host's normal state, never an error — while an explicitly named `--transcript` pointing at a session file from a harness Rasen ships no reader for exits **non-zero** naming that harness. Naming a file is an input, and reading one harness's file with another harness's field names would report a confident number that describes nothing. The whole handoff mechanism is built on this probe + the principle of "discrete checkpoints, never injecting a persistent countdown":
