...
- **Where the contract lives**: `propose` produces `proposal.md` / `design.md` / `specs/<cap>/spec.md` / `tasks.md` in `rasen/changes/<id>/`. This is the "truth" handed off between stages.
- **Definition of done**: every `### Requirement` must have at least one `#### Scenario` (enforced by `rasen validate`). The verify/review stages check the implementation against the scenarios.
...
## 2. Run the entire workflow with one command: `/rasen-auto`

`/rasen-auto` (Autopilot) is the **single-command end-to-end** entry point. It turns the executor into a **LEAD**: the LEAD only orchestrates and does not do the stage work itself — it **classifies the task → picks a pipeline → dispatches each stage to a role-isolated sub-agent → pauses at gates for confirmation**. You can interrupt and switch to manual at any time.
...
```bash
rasen pipeline classify "<task description>" --json   # → { suggested, matched, available }
rasen pipeline show <name> --json             # → { name, description, buildOrder, stages }
rasen pipeline list --json                     # list all pipelines from package/user/project
```
...
- **Default**: `/rasen-auto <task>` (without an explicit choice) → use **`small-feature`** directly, no auto-upgrade to full-feature/bug-fix.
...
- **The parent change becomes a planning container.** Its own remaining stages are marked delegated (not run at the parent level); each sub-change is created with `rasen new change <child-id>`, running the resolved `childPipeline` (default `small-feature`, never contains decompose). **Per-sub-change pipeline override is allowed** — one sub can be `bug-fix` while a sibling is `full-feature`.
...
| Need validation | `/rasen-office-hours-command` | YC-style need validation (Startup six questions / Builder brainstorm) | `office-hours-design.md` |
...
| Kickoff (fine-grained) | `/rasen-new-change` → `/rasen-continue-change` (repeat) | One artifact at a time, generating the next by dependency | Same as above, in steps |
| Implement | `/rasen-apply-change` | Implement per `tasks.md`, checking off items | Code + checked-off tasks |
| Verify | `/rasen-verify-change` | Check that the implementation matches the artifacts (spec scenarios) | Verification conclusion |
...
### 3.1 `/rasen-explore` — think it through first
An unstructured exploration conversation: browse code, compare options, sketch diagrams. Once an idea takes shape you can transition to `/rasen-propose` (default) or `/rasen-new-change` (expanded).

### 3.2 `/rasen-office-hours-command` — validate whether the need is worth doing first
...
- **Existing active change**: written to `rasen/changes/<id>/office-hours-design.md` (fixed name within the task directory, same as `proposal.md`; auto-consumed by propose).
...
### 3.3 `/rasen-propose` — kickoff + generate planning artifacts in one step
Create `rasen/changes/<id>/` and generate all the artifacts needed before implementation (spec-driven: proposal → specs → design → tasks), stopping at the "ready to apply" state. For stepwise control use expanded `/rasen-new-change` + `/rasen-continue-change`.

### 3.4 `/rasen-apply-change` — implement
...
### 3.5 `/rasen-review-cycle` — iterative review loop (also `/rasen-auto`'s review-loop stage)
...
### 3.6 `/rasen-verify-change` / `/rasen-verify-enhanced` — verify
...
### 3.7 Context awareness and handoff (`rasen agent context` + `/rasen-handoff`)

An agent can't perceive its own context usage — it can only **measure** it. `rasen agent context` reads the precise usage from the API usage recorded in the transcript (`--latest` measures the main session itself, `--transcript <path>` measures a specific worker, `--json` outputs `{ model, window, contextTokens, limit, pct, remainingTokens, shouldHandoff }`). The two flags fail differently, on purpose: a bare `--latest` on a host whose harness has no context-probe adapter answers `{ available: false, reason: "unsupported-host" }` at exit 0 — that host's normal state, never an error — while an explicitly named `--transcript` pointing at a session file from a harness Rasen ships no reader for exits **non-zero** naming that harness. Naming a file is an input, and reading one harness's file with another harness's field names would report a confident number that describes nothing. For the same reason the receipt withholds a verdict instead of guessing one: when the model's context window can't be resolved it reports `window: "unknown"` and **omits `shouldHandoff` entirely**, and `limit` / `pct` / `remainingTokens` are then placeholders rather than measurements — so a consumer branches on the **presence** of `shouldHandoff`, never on its falsiness. The whole handoff mechanism is built on this probe + the principle of "discrete checkpoints, never injecting a persistent countdown":

- **Session level**: `/rasen-handoff` is available anytime — probes, writes `rasen/changes/<id>/handoff/lead-<n>.md` (original intent / key decisions / dead ends / next step), and records the `sessionHandoff` pointer (with its relay generation `n`) in `auto-run.json`. The `/rasen-auto` entry does a non-blocking pre-check: at or above the threshold it offers a three-way choice — automatic relay now / continue this session / handle manually — and the user decides. Not handing off is fine — the harness's auto-compact is the fallback.
- **Session relay (active successor launch)**: with the user's authorization, the exhausted session launches its own successor — a visible interactive Claude Code window in the project root, bootstrapped to read the handoff doc, run `rasen pipeline resume`, and continue from the documented next action. Invariants: relay only at a stage boundary (all workers returned, run-state persisted); spawn strictly after the doc + run-state hit disk, then the predecessor stands down; the bootstrap prompt travels via file indirection (`handoff/relay-prompt.txt`) or PowerShell `-EncodedCommand` — bare-quoted prompts get truncated by nested shell parsing; `sessionHandoff.n` at `maxRelays` stops auto-relay and recommends decomposing instead. Subagents are never resumed across sessions — the successor re-creates workers from the handoff doc / recorded transcripts / change directory.
- **Compact recovery hook (passive reinforcement)**: an optional `SessionStart` hook (matcher `compact`, script `hooks/compact-recovery.sh`; `rasen init` prints the copy-paste snippet and never edits `.claude/settings.json` itself) injects guidance right after an auto-compaction: run `rasen pipeline resume`, read the handoff distillates first, and don't trust fine-grained details from the machine summary. Complementary to active relay — same recovery entry point, no second state channel.
- **Worker level (automatic)**: every dispatch prompt carries a handoff clause — when the worker hits a soft budget / loses recall of details it read earlier (self-assessment), it writes `handoff/<role>-<n>.md` (fixer / debugger must include an "eliminated hypotheses and evidence" section), returns a structured `HANDOFF {path, reason, completed, remaining}`; the LEAD accounts for it (the stage's `handoffs[]`, single-writer invariant) and dispatches a successor to continue within the same session, without interrupting the pipeline. Before each `SendMessage` continuation (re-review / planner reuse) the LEAD also probes that worker first, and if over threshold "writes a handoff doc → retires and replaces".
- **Codex worker handoff — known limitation.** The H.2 warm-continue guard is playbook-driven: the LEAD runs the per-continuation transcript probe itself (`rasen agent context --transcript <rollout> --runtime codex`, which reads real occupancy), it is not a code-enforced gate, so a busy LEAD can skip it and over-reuse a worker (observed: one Codex thread reused ~15× over 8h). The probe machinery is sound — the gap is execution reliability. This is tolerated on Codex because a worker continuing through the host's low-loss auto-compact (rather than handing off) is the intended behavior there, and the Codex handoff default is already looser (0.85 vs claude's 0.5).
...
- **Tunable config + defaults** — all orchestration tunables live in `pipeline.yaml` (built-ins under `pipelines/<name>/pipeline.yaml`; resolution priority `project > user > package`, so a same-named file overrides the built-in). Inspect the **resolved** values any time with `rasen pipeline show <name> --json`.
  ...
  **Capability tier** (auto-detected, not set in YAML): set `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` for Tier A (warm `SendMessage` continuation — the default, since `rasen init`/`update` merges this flag into the project's `.claude/settings.json`). Absent it, Tier B (fresh worker per stage) is used.

  **Hooks** (optional, opt-in via the `rasen init` copy-paste snippets — never auto-written to `.claude/settings.json`): `hooks/safety-check.sh` (`PreToolUse`, blocks destructive commands) and `hooks/compact-recovery.sh` (`SessionStart`, matcher `compact`, re-anchors on handoff docs after a compaction).
...
| Command | Use |
|---|---|
| `rasen init [path] --tools <list>` | Initialize; generate skills/commands per AI tool |
| `rasen update` | Refresh generated instruction files after a CLI upgrade |
| `rasen new change <name> [--schema <s>]` | Create a change directory + `.openspec.yaml` |
| `rasen status --change <id> [--json]` | Show an artifact-completion status for a change (done/total/blocked) |
| `rasen instructions [artifact] --change <id> [--json]` | Output the generation instructions for an artifact (this is how slash commands work) |
| `rasen list [--specs] [--json]` | List changes or specs |
| `rasen show [item] [--json] [--deltas-only]` | Show a change/spec |
| `rasen validate [item] [--all\|--changes\|--specs\|--pipelines] [--strict] [--json]` | Validate structure / scenarios / archive-safety (including pipeline definitions) |
| `rasen pipeline <list\|show <name>\|classify "<task>"\|resume <change>> [--json]` | Data-driven pipeline registry: list / view DAG / classify task / resume (`auto` fetches the pipeline from this) |
| `rasen archive <change> [--skip-specs] [--no-validate]` | Archive + merge deltas into canonical specs |
| `rasen templates / schemas [--json]` | View artifact-template paths / available schemas |
| `rasen config <list\|profile\|edit>` | View / switch profile and delivery |
| `rasen schema <init\|fork\|validate\|which>` | Manage custom workflow schemas |
...
For projects that have already run an older `rasen init`, **don't** rerun init — use **`rasen update`**:
...
2. **Refresh the generated artifacts in the project**:
   ```bash
   rasen update          # regenerate .claude/skills + commands per the configured tools/profile/delivery; includes legacy migration
   ```
...
3. **The new `rasen pipeline` CLI and built-in pipelines ship with the package** — they're **immediately available** in the upgraded binary, with nothing to generate into the project.
4. If you were previously on `core` profile and want to enable this release's opt-in workflows (`review-cycle` / fusion `auto`, etc.): first `rasen config profile` to re-select, then `rasen update`.
...
```text
You: /rasen-auto Add an "export all data" feature to the settings page
...
/rasen-explore How to do mobile auth

# 2) Kickoff (generate proposal/design/specs/tasks)
/rasen-propose add-jwt-auth
rasen status --change add-jwt-auth        # check artifact completion
...
| I want to… | Use |
|---|---|
| Run end-to-end with one command | `/rasen-auto <task>` (default small-feature pipeline) |
| Specify a particular pipeline | `/rasen-auto --pipeline <name> <task>` or `/rasen-auto <name> <task>` |
| See which pipelines exist | `rasen pipeline list` |
| Think it through before acting | `/rasen-explore` |
| Validate whether the need is worth doing | `/rasen-office-hours-command` |
| Kickoff + generate a plan | `/rasen-propose` (fine-grained: `/rasen-new-change` + `/rasen-continue-change`) |
| Implement | `/rasen-apply-change` |
...
| Recognized, no dispatch adapter (`zed`, `omp`) | compatible legacy target | `legacy-fallback` | non-fatal warning naming the host; stages report `runtimeSource: legacy-default`, not `host` |
...
rasen pipeline agents small-feature --json
...
## 9. Goal-driven iteration: `/rasen-goal`

`/rasen-auto` assumes the product is a single reviewable code change (propose → apply → verify → ship). Some tasks don't fit that shape — their "done" is a **condition**, not a document: drive a Lighthouse score to 90, make a module rubric-clean, research and write a brief. `/rasen-goal` is the entry point for those: it repeats **modify → judge** until a gate is satisfied or a round cap is hit.

> Use `/rasen-goal` when the product is a *condition* met by iteration. Use `/rasen-auto` when the product is a code-change document. The two share the same orchestration playbook (LEAD + role-isolated workers, tiers, run-state, gates, resume) — `/rasen-goal` is a sibling entry, not a second system.
...
```text
/rasen-goal <task>                        # LEAD classifies by keyword
/rasen-goal measure <task>                # force the measure variant
/rasen-goal evaluate <task>               # force the evaluate variant
/rasen-goal research <task>               # force the research variant
/rasen-goal --pipeline goal-loop-<variant> <task>   # explicit pipeline name
```
...
```bash
rasen pipeline show goal-loop-measure        # view the DAG + the loop metadata
rasen pipeline show goal-loop-measure --json  # { name, buildOrder, stages }
```
...
```bash
rasen pipeline resume <change> --json   # next incomplete stage + worker pointers
```
...
```text
You: /rasen-goal make the auth module error-handling satisfy this rubric
...
```text
You: /rasen-goal research and write a brief on WebGPU compute adoption
