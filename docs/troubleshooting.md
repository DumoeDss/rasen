...
## Learned-skill materialization

### "Learned effective conflict"

Two or more member stores publish the same applicable learned-skill ID but do
not have the same stable knowledge key and exact canonical bytes. Rasen will not
choose by registry order, path, planning store, or config pointer. Review every
typed source/key/digest in the diagnostic, then align the canonical records,
rename one ID, or retire the revision that should not participate. Re-run
`rasen update`; no project-local learned file or ledger was changed by the
blocked run.

### "Unavailable store" or "Deferred remove/replace"

The store registry entry could not be verified while prior ownership, the
project's `store:` pointer, or frozen planning facts say it may affect this
project. Repair the registration/root and run:

```bash
rasen store doctor <store-id>
rasen update
```

Rasen preserves uncertain exact copies until the store can be evaluated. An
unrelated unavailable store is reported but does not block unaffected learned
additions, and a deterministic project-scoped winner can still replace an
unknown lower store source.

### A learned copy was not refreshed or removed

Check the path named in the diagnostic. Rasen manages only exact regular files
recorded in `rasen/.learned-skill-materializations.json`; human edits,
untracked directories, symlinks/reparse points, non-regular occupants, missing
copies, and tampered ledger paths are never overwritten or deleted. Move your
content to a different ID or restore the exact generated bytes before retrying.
