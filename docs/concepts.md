...
- **Workflow — the inner loop.** A workflow is one task unit that runs inside one session: an AI code agent plans and executes it, possibly dispatching subagents along the way, and comes back with a result. `rasen-propose`, `rasen-apply-change`, `rasen-review-cycle` — each is a single inner-loop task. `rasen workflow list` shows the catalog of these installable units.
...
- **Trust boundary.** A shared workflow or pipeline is an executable prompt: importing one means an AI agent will read and act on its contents. Rasen doesn't solve this with signatures. It mitigates it with a transactional install (nothing is written until validation passes), a content digest (so a reinstall or update can prove nothing changed underneath you), static `validate` (checked before anything is installed), and `rasen-workflow-author`'s bundled independent-review branch before a package is trusted. Review the source before you import it — the same way you'd review a dependency before adding it to a project.

### Where this is heading

The unified catalog now includes 12 built-in experts (`rasen-review`, `rasen-qa`, and friends) under `kind: 'expert'`, with declared workflow dependencies and digest-tracked sidecars. Workflows and pipelines are installable/exportable `.rasenpkg` units; the same trust boundary still applies to every imported prompt package.
